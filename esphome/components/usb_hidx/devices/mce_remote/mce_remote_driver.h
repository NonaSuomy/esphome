...
class MCERemoteDriver : public HIDDeviceDriver {
 public:
  ...
  HIDDeviceDriver *clone() const override { return new MCERemoteDriver(*this); }

  bool match_device(uint8_t protocol, uint16_t vid, uint16_t pid) override {
    if (vid == 0x1784)
      return true;
    if (vid == 0x045E && (pid == 0x006D || pid == 0x00A0 || pid == 0x00F2 || pid == 0x0745))
      return true;
    if (vid == 0x0471 && (pid == 0x0608 || pid == 0x060C || pid == 0x060D || pid == 0x060F || pid == 0x0613 ||
                          pid == 0x0815 || pid == 0x206C || pid == 0x2088 || pid == 0x2093))
      return true;
    if (vid == 0x0609 &&
        (pid == 0x031D || pid == 0x0322 || pid == 0x0334 || pid == 0x0338 || pid == 0x0353 || pid == 0x0357))
      return true;
    if (vid == 0x1460 && pid == 0x9150)
      return true;
    if (vid == 0x1308 && pid == 0xC001)
      return true;
    if (vid == 0x051C && pid == 0xC001)
      return true;
    if (vid == 0x107B && pid == 0x3009)
      return true;
    if (vid == 0x03EE && pid == 0x2501)
      return true;
    if (vid == 0x179D && pid == 0x0010)
      return true;
    if (vid == 0x195D && pid == 0x7002)
      return true;
    if (vid == 0x1509 && pid == 0x9242)
      return true;
    if (vid == 0x043E && pid == 0x9803)
      return true;
    if (vid == 0x147A && (pid == 0xE015 || pid == 0xE016 || pid == 0xE017 || pid == 0xE018 || pid == 0xE03A ||
                          pid == 0xE03C || pid == 0xE03E || pid == 0xE042))
      return true;
    if (vid == 0x1934 && (pid == 0x5168 || pid == 0x0602 || pid == 0x0702))
      return true;
    if (vid == 0x2304 && pid == 0x0225)
      return true;
    if (vid == 0x1019 && pid == 0x0F38)
      return true;
    if (vid == 0x0FB8 && pid == 0x0002)
      return true;
    if (vid == 0x185B && (pid == 0x3020 || pid == 0x3082))
      return true;
    if (vid == 0x04EB && pid == 0xE004)
      return true;
    if (vid == 0x105A && pid == 0x2000)
      return true;
    if (vid == 0x0572 && (pid == 0x58A1 || pid == 0x58A5))
      return true;
    if (vid == 0x0BDA && pid == 0x0161)
      return true;
    if (vid == 0x2596 && (pid == 0x8008 || pid == 0x8016 || pid == 0x8042))
      return true;
    if (vid == 0x03F3 && pid == 0x0094)
      return true;
    return false;
  }
 ...
 protected:
  ...
  void emit_if_ready() {
    ...
    if (count_ >= 32 && (sc & 0xFFFF0000) == 0x800F0000) {
      ...
      sc &= ~0x8000U;
      uint8_t cmd = sc & 0xFF;
      ESP_LOGI("usb_hidx.mce", "MCE key: cmd=0x%02X toggle=%d scancode=0x%08X", cmd, tog, static_cast<unsigned>(sc));
