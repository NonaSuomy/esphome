...
class SwitchDriver : public HIDDeviceDriver {
 public:
  ...
  HIDDeviceDriver *clone() const override { return new SwitchDriver(*this); }
    ...
    if (is_powera_) {
      ...
      if (btn0 != last_btn0_) {
        ...
        if ((btn0 & 0x02) && !(last_btn0_ & 0x02)) {
          ...
          if (parent_->gamepad_button_cross_sensor_)
            parent_->gamepad_button_cross_sensor_->publish_state(true);
        }
        if ((btn0 & 0x04) && !(last_btn0_ & 0x04)) {
          ...
          if (parent_->gamepad_button_circle_sensor_)
            parent_->gamepad_button_circle_sensor_->publish_state(true);
        }
        ...
        if ((btn0 & 0x40) && !(last_btn0_ & 0x40)) {
          ...
          if (parent_->gamepad_button_zl_sensor_)
            parent_->gamepad_button_zl_sensor_->publish_state(true);
        }
        if ((btn0 & 0x80) && !(last_btn0_ & 0x80)) {
          ESP_LOGI("usb_hidx.switch", "ZR Button");
          if (parent_->gamepad_button_zr_sensor_)
            parent_->gamepad_button_zr_sensor_->publish_state(true);
        }
        if (!(btn0 & 0x01) && (last_btn0_ & 0x01)) {
          if (parent_->gamepad_button_x_sensor_)
        ...
        if (!(btn0 & 0x02) && (last_btn0_ & 0x02)) {
          ...
          if (parent_->gamepad_button_cross_sensor_)
            parent_->gamepad_button_cross_sensor_->publish_state(false);
        }
        if (!(btn0 & 0x04) && (last_btn0_ & 0x04)) {
          if (parent_->gamepad_button_b_sensor_)
            parent_->gamepad_button_b_sensor_->publish_state(false);
          if (parent_->gamepad_button_circle_sensor_)
            parent_->gamepad_button_circle_sensor_->publish_state(false);
        }
        if (!(btn0 & 0x08) && (last_btn0_ & 0x08)) {
          if (parent_->gamepad_button_y_sensor_)
        ...
        if (!(btn0 & 0x40) && (last_btn0_ & 0x40)) {
          if (parent_->gamepad_button_zl_sensor_)
            parent_->gamepad_button_zl_sensor_->publish_state(false);
        }
        if (!(btn0 & 0x80) && (last_btn0_ & 0x80)) {
          if (parent_->gamepad_button_zr_sensor_)
            parent_->gamepad_button_zr_sensor_->publish_state(false);
        }
        last_btn0_ = btn0;
      }
      ...
      if (btn_shared != last_btn1_) {
        ...
        if ((btn_shared & 0x08) && !(last_btn1_ & 0x08)) {
          ESP_LOGI("usb_hidx.switch", "L3");
          if (parent_->gamepad_button_l3_sensor_)
            parent_->gamepad_button_l3_sensor_->publish_state(true);
        }
        if ((btn_shared & 0x04) && !(last_btn1_ & 0x04)) {
          ESP_LOGI("usb_hidx.switch", "R3");
          if (parent_->gamepad_button_r3_sensor_)
            parent_->gamepad_button_r3_sensor_->publish_state(true);
        }
        ...
        if ((btn_shared & 0x20) && !(last_btn1_ & 0x20)) {
          ESP_LOGI("usb_hidx.switch", "Capture");
          if (parent_->gamepad_button_capture_sensor_)
            parent_->gamepad_button_capture_sensor_->publish_state(true);
        }
        if (!(btn_shared & 0x20) && (last_btn1_ & 0x20)) {
          if (parent_->gamepad_button_capture_sensor_)
            parent_->gamepad_button_capture_sensor_->publish_state(false);
        }
      ...
      if (dpad_pro != (last_btn_left_ & 0x0F)) {
        if (parent_->gamepad_dpad_up_sensor_)
          parent_->gamepad_dpad_up_sensor_->publish_state(btn_left & 0x02);
        if (parent_->gamepad_dpad_down_sensor_)
          parent_->gamepad_dpad_down_sensor_->publish_state(btn_left & 0x01);
        if (parent_->gamepad_dpad_left_sensor_)
          parent_->gamepad_dpad_left_sensor_->publish_state(btn_left & 0x08);
        if (parent_->gamepad_dpad_right_sensor_)
          parent_->gamepad_dpad_right_sensor_->publish_state(btn_left & 0x04);
      }
      ...
      if (abs((int) lx - (int) last_lx_) > 50 || abs((int) ly - (int) last_ly_) > 50) {
        ...
        if (parent_->get_gamepad_left_stick_x_sensor())
          parent_->get_gamepad_left_stick_x_sensor()->publish_state(lx_f);
        if (parent_->get_gamepad_left_stick_y_sensor())
          parent_->get_gamepad_left_stick_y_sensor()->publish_state(ly_f);
        last_lx_ = lx;
        last_ly_ = ly;
      }
      if (abs((int) rx - (int) last_rx_) > 50 || abs((int) ry - (int) last_ry_) > 50) {
        ...
        if (parent_->get_gamepad_right_stick_x_sensor())
          parent_->get_gamepad_right_stick_x_sensor()->publish_state(rx_f);
        if (parent_->get_gamepad_right_stick_y_sensor())
          parent_->get_gamepad_right_stick_y_sensor()->publish_state(ry_f);
        last_rx_ = rx;
        last_ry_ = ry;
      }
      ...
      if (len >= 49) {
        ...
        if (now - last_imu_pub > 100) {
          last_imu_pub = now;
          char buf[64];
          snprintf(buf, sizeof(buf), "A:%.1f,%.1f,%.1f G:%.0f,%.0f,%.0f", ax_g, ay_g, az_g, gx_d, gy_d, gz_d);
 ...
 public:
  ...
  void send_rumble(HIDDevice *device, uint8_t left_strength, uint8_t right_strength) override {
    if (device == nullptr)
      device = device_;
    if (device == nullptr)
      return;
  ...
  void send_subcommand(HIDDevice *device, uint8_t subcommand, const uint8_t *data, size_t len) {
    uint8_t buf[64] = {0};
    if (len > sizeof(buf) - 11) {
      ESP_LOGW("usb_hidx.switch", "Subcommand 0x%02X payload too large: %u", subcommand, len);
      return;
    }
