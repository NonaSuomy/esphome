...
class Xbox360Driver : public HIDDeviceDriver {
 public:
  ...
  HIDDeviceDriver *clone() const override { return new Xbox360Driver(*this); }

  void on_device_ready(HIDDevice *device) override {
    this->set_device(device);
    this->init_controller(device);
  }

  void on_device_removed() override { this->device_ = nullptr; }
  ...
  bool match_device(uint8_t protocol, uint16_t vid, uint16_t pid) override {
    ...
    is_gamepad_ = (vid == 0x045E && (pid == 0x028E || pid == 0x0719)) || (vid == 0x2DC8 && pid == 0x310B);
 ...
 public:
  ...
  void send_rumble(HIDDevice *device, uint8_t left_motor, uint8_t right_motor) override {
    if (!device && device_)
      device = device_;
    if (!device)
      return;
 ...
 protected:
  ...
  void process_common_gamepad(uint8_t buttons_lo, uint8_t buttons_hi, uint8_t lt, uint8_t rt, int16_t lx, int16_t ly,
                              int16_t rx, int16_t ry) {
    bool is_idle = (buttons_lo == 0 && buttons_hi == 0 && lt < 30 && rt < 30 && abs(lx) < 8000 && abs(ly) < 8000 &&
    ...
    if (nav != last_buttons2_) {
      ...
      if ((nav & 0x10) && !(last_buttons2_ & 0x10))
        ESP_LOGI("usb_hidx.xbox360", "Button Start pressed");
      if ((nav & 0x20) && !(last_buttons2_ & 0x20))
        ESP_LOGI("usb_hidx.xbox360", "Button Back pressed");
      if ((nav & 0x40) && !(last_buttons2_ & 0x40))
        ESP_LOGI("usb_hidx.xbox360", "Button L3 pressed");
      if ((nav & 0x80) && !(last_buttons2_ & 0x80))
        ESP_LOGI("usb_hidx.xbox360", "Button R3 pressed");
      last_buttons2_ = nav;
    }
    ...
    if (buttons_hi != last_buttons1_) {
      ...
      if ((buttons_hi & 0x01) && !(last_buttons1_ & 0x01))
        ESP_LOGI("usb_hidx.xbox360", "Button LB pressed");
      if ((buttons_hi & 0x02) && !(last_buttons1_ & 0x02))
        ESP_LOGI("usb_hidx.xbox360", "Button RB pressed");
      if ((buttons_hi & 0x04) && !(last_buttons1_ & 0x04))
        ESP_LOGI("usb_hidx.xbox360", "Button Guide pressed");
      if ((buttons_hi & 0x10) && !(last_buttons1_ & 0x10))
        ESP_LOGI("usb_hidx.xbox360", "Button A pressed");
      if ((buttons_hi & 0x20) && !(last_buttons1_ & 0x20))
        ESP_LOGI("usb_hidx.xbox360", "Button B pressed");
      if ((buttons_hi & 0x40) && !(last_buttons1_ & 0x40))
        ESP_LOGI("usb_hidx.xbox360", "Button X pressed");
      if ((buttons_hi & 0x80) && !(last_buttons1_ & 0x80))
    ...
    if (axis_lx_sensor_)
      axis_lx_sensor_->publish_state(lx_f);
    if (axis_ly_sensor_)
      axis_ly_sensor_->publish_state(ly_f);
    if (axis_rx_sensor_)
      axis_rx_sensor_->publish_state(rx_f);
    if (axis_ry_sensor_)
      axis_ry_sensor_->publish_state(ry_f);
#endif
    if (abs((int) lt - (int) last_lt_) > 10) {
      ESP_LOGI("usb_hidx.xbox360", "Left Trigger: %d", lt);
      last_lt_ = lt;
    }
    if (abs((int) rt - (int) last_rt_) > 10) {
  ...
  void process_instrument_report(const uint8_t *data, size_t len) {
    ...
    if (buttons1 != last_buttons1_) {
      if ((buttons1 & 0x10) && !(last_buttons1_ & 0x10))
        ESP_LOGI("usb_hidx.xbox360", "%s: A", prefix);
      if ((buttons1 & 0x20) && !(last_buttons1_ & 0x20))
        ESP_LOGI("usb_hidx.xbox360", "%s: B", prefix);
      if ((buttons1 & 0x40) && !(last_buttons1_ & 0x40))
        ESP_LOGI("usb_hidx.xbox360", "%s: X", prefix);
      if ((buttons1 & 0x80) && !(last_buttons1_ & 0x80))
        ESP_LOGI("usb_hidx.xbox360", "%s: Y", prefix);
    ...
    if (is_guitar_) {
      ...
      if (abs(rx - last_whammy_) > 1000) {
        ESP_LOGI("usb_hidx.xbox360", "Guitar: Whammy=%d", rx);
        last_whammy_ = rx;
      }
    }
  }
