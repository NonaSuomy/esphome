...
BASE_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_ACCEL_ODR, default="104HZ"): cv.enum(ODRS, upper=True),
    cv.Optional(CONF_GYRO_ODR, default="104HZ"): cv.enum(ODRS, upper=True),
...
async def to_code(config):
    ...
    accel_odr = ODRS[config[CONF_ACCEL_ODR]]
    gyro_odr = ODRS[config[CONF_GYRO_ODR]]
