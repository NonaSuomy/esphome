...
// Register addresses
enum class VL6180XRegister : uint16_t {
  ...
  VL6180X_REG_RANGE_SCALER = 0x096,

  VL6180X_REG_SYSRANGE_CROSSTALK_VALID_HEIGHT = 0x021,

  VL6180X_REG_SYSRANGE_RANGE_CHECK_ENABLES = 0x02D,
  // Trigger Ranging
...
// Scalar
enum class VL6180XScalerFactor : uint8_t {
  // Sensor scale factor
  // The higher the value the less acurate.
  VL6180X_SCALER_FACTOR_1 = 0xFD, // 1x Scaler Factor 0mm to 20mm with a granularity of 1mm
  VL6180X_SCALER_FACTOR_2 = 0x7F, // 2x Scaler Factor 0mm to 40mm with a granularity of 2mm
  VL6180X_SCALER_FACTOR_3 = 0x54, // 3x Scaler Factor 0mm to 60mm with a granularity of 3mm
};

// ALS Sensor gain levels
enum class VL6180XALSGain {
  // Sensor gain levels
...
enum VL6180XError {
  ...
  VL6180X_ERROR_SENSORVERSIONNOTFOUND_19 = 19,  // Custom error to show no version found.
};
...
class VL6180XSensor : public sensor::Sensor, public PollingComponent, public i2c::I2CDevice {
  public:
    ...
    uint8_t get_scaling() const { return scaling_; }
    ...
    void set_gain(VL6180XALSGain gain) { gain_ = gain; }
...
/*     Trigger<> *get_gesture_swipe_toward_trigger() const { return on_gesture_swipe_toward_trigger_; }
  ...
  private:
    uint8_t scaling_{1};  // Default to 1x scaling
    int8_t ptp_offset_{0};  // Part-to-part offset
    static const uint16_t ScalerValues[4];
    uint8_t get_scaler_value() const {
      switch (scaling_) {
        case 1: return static_cast<uint8_t>(VL6180XScalerFactor::VL6180X_SCALER_FACTOR_1);
        case 2: return static_cast<uint8_t>(VL6180XScalerFactor::VL6180X_SCALER_FACTOR_2);
        case 3: return static_cast<uint8_t>(VL6180XScalerFactor::VL6180X_SCALER_FACTOR_3);
        default: return static_cast<uint8_t>(VL6180XScalerFactor::VL6180X_SCALER_FACTOR_1);  // Default to 1x if invalid
      }
    }
  ...
  protected:
    ...
    VL6180XALSGain gain_{VL6180XALSGain::VL6180X_ALS_GAIN_1}; // Initialize with default gain
    sensor::Sensor *distance_sensor_;
    sensor::Sensor *als_sensor_;
    float lux_without_glass_;
    float als_lux_resolution_without_glass_;
    float read_als(); // Update the function signature
