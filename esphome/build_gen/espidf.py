...
def get_project_cmakelists(
    ...
    # Keep project-local ESP-IDF component overrides available to the generated
    # project.  This is used by the USB host TT work without modifying the
    # PlatformIO framework cache.  Resolve it from the YAML configuration
    # directory so a copied ESPHome checkout remains relocatable.
    idf_override_dir = CORE.config_dir / "idf_components"
    extra_component_dirs = '"${CMAKE_SOURCE_DIR}/src"'
    if idf_override_dir.is_dir():
        extra_component_dirs += f' "{idf_override_dir}"'
...
# LVGL 9.5's CMake source list includes the ARM Helium software-blend
# implementation unconditionally.  ESP32-P4 is RISC-V, so GCC interprets
# that .S file as RISC-V assembly and fails on the ARM C typedefs in the
# preprocessed input.  PlatformIO's source filter cannot affect a native
# ESP-IDF build; remove the source from the already-registered LVGL target
# here instead.  Keep this in the generated project so Component Manager can
# freely refresh its cached LVGL component without losing the workaround.
if(IDF_TARGET STREQUAL "esp32p4" AND TARGET __idf_lvgl)
    get_target_property(_esphome_lvgl_sources __idf_lvgl SOURCES)
    if(_esphome_lvgl_sources)
        list(FILTER _esphome_lvgl_sources EXCLUDE REGEX "lv_blend_helium\\.S$")
        set_property(TARGET __idf_lvgl PROPERTY SOURCES "${{_esphome_lvgl_sources}}")
    endif()
endif()
