 ...
 */
typedef enum { PORT_DEV_NOT_PRESENT = 0, PORT_DEV_PRESENT } port_dev_state_t;
...
typedef struct {
  ...
  } single_thread; /**< Single thread members don't require a critical section so long as they are never accessed from
                      multiple threads */
...
static esp_err_t port_request_status(ext_port_t *ext_port) {
  ext_port_parent_request_data_t data = {.type = EXT_PORT_PARENT_REQ_CONTROL,
                                         .control = {
                                             .req = USB_B_REQUEST_HUB_GET_PORT_STATUS,
                                         }};

  esp_err_t ret = p_ext_port_driver->constant.hub_request_cb((ext_port_hdl_t) ext_port, &data,
...
static void port_stop_handling(ext_port_t *ext_port) {
  ...
  if (ext_port->action_flags) {
    ...
    ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Port stop handling. Dropped actions 0x%" PRIx32 "",
             ext_port->constant.parent_dev_addr, ext_port->constant.port_num, ext_port->action_flags);
  }
...
static void port_event(ext_port_t *ext_port, ext_port_event_t event) {
  ...
  p_ext_port_driver->constant.event_cb((ext_port_hdl_t) ext_port, &event_data,
                                       p_ext_port_driver->constant.event_cb_arg);
}
 ...
 * @return
 *    - ESP_ERR_INVALID_ARG:    Unable to allocate the port object: parent hub handle and parent device handle must be
 * not NULL
 ...
 */
static esp_err_t port_alloc(void *context, usb_device_handle_t parent_dev_hdl, uint8_t parent_port_num,
                            uint16_t port_delay_ms, ext_port_t **port_obj) {
  ...
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Port has been added (PwrOn2PwrGood=%d ms)", ext_port->constant.parent_dev_addr,
           ext_port->constant.port_num, ext_port->constant.power_on_delay_ms);
...
static esp_err_t handle_complete(ext_port_t *ext_port) {
  ...
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Port is %s", ext_port->constant.parent_dev_addr, ext_port->constant.port_num,
           ext_port->flags.is_gone                     ? "gone"
           : (ext_port->dev_state == PORT_DEV_PRESENT) ? "active"
                                                       : "idle");
  ...
  if (all_ports_were_handled) {
    ...
    ESP_ERROR_CHECK(p_ext_port_driver->constant.hub_request_cb((ext_port_hdl_t) ext_port, &data,
                                                               p_ext_port_driver->constant.hub_request_cb_arg));
  }
...
static bool handle_port_status(ext_port_t *ext_port) {
  ...
  if (port_is_in_reset(ext_port)) {
    ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Port still in reset, wait and repeat get status...",
             ext_port->constant.parent_dev_addr, ext_port->constant.port_num);
...
static void handle_port_connection(ext_port_t *ext_port) {
  ...
  ESP_LOGI(EXT_PORT_TAG, "[%d:%d] Downstream %s (status=0x%04x, change=0x%04x)", ext_port->constant.parent_dev_addr,
           ext_port->constant.port_num, port_has_connection(ext_port) ? "device connected" : "device disconnected",
           ext_port->status.wPortStatus.val, ext_port->status.wPortChange.val);
  ...
  switch (ext_port->state) {
    ...
    case USB_PORT_STATE_DISABLED:
      if (port_has_connection(ext_port)) {
        if (ext_port->dev_state == PORT_DEV_PRESENT) {
          ESP_LOGE(EXT_PORT_TAG, "[%d:%d] Mismatch port state and device status", ext_port->constant.parent_dev_addr,
    ...
    case USB_PORT_STATE_RESETTING:
      if (!port_has_connection(ext_port)) {
        if (ext_port->dev_state == PORT_DEV_PRESENT) {
          ESP_LOGE(EXT_PORT_TAG, "[%d:%d] Failed to issue downstream port reset", ext_port->constant.parent_dev_addr,
                   ext_port->constant.port_num);
          has_device = true;
        } else {
          ext_port->state = USB_PORT_STATE_DISCONNECTED;
...
static bool handle_port_changes(ext_port_t *ext_port) {
  ...
  if (port_has_changed_connection(ext_port)) {
    ...
    ESP_LOGE(EXT_PORT_TAG, "[%d:%d] Error on port (state=%d, dev=%d)", ext_port->constant.parent_dev_addr,
             ext_port->constant.port_num, ext_port->state, ext_port->dev_state == PORT_DEV_PRESENT);
    ...
    case USB_PORT_STATE_DISCONNECTED:
      if (port_has_connection(ext_port)) {
        if (ext_port->dev_reset_attempts < EXT_PORT_RESET_ATTEMPTS) {
          ...
          ESP_LOGE(EXT_PORT_TAG, "[%d:%d] Unable to reset the device", ext_port->constant.parent_dev_addr,
                   ext_port->constant.port_num);
        }
      }
      break;
        ...
        if (port_has_connection(ext_port)) {
          if (ext_port->dev_state == PORT_DEV_NOT_PRESENT) {
            ...
            if (ext_port->status.wPortStatus.PORT_LOW_SPEED == 0) {
              dev_speed = ext_port->status.wPortStatus.PORT_HIGH_SPEED ? USB_SPEED_HIGH : USB_SPEED_FULL;
            }
            ESP_LOGD(EXT_PORT_TAG, "Device speed %s", (char *[]){"Low", "Full", "High"}[dev_speed]);
            ESP_LOGI(EXT_PORT_TAG, "[%d:%d] Downstream device attached, speed=%s", ext_port->constant.parent_dev_addr,
                     ext_port->constant.port_num, (char *[]){"Low", "Full", "High"}[dev_speed]);
          ...
          // TODO: IDF-10071 Port debounce mechanism
          ESP_LOGE(EXT_PORT_TAG, "[%d:%d] Enabled, but doesn't have connection", ext_port->constant.parent_dev_addr,
                   ext_port->constant.port_num);
        }
      } else {
        // If port was enabled, there should be an active device
        if (ext_port->dev_state == PORT_DEV_PRESENT) {
          ESP_LOGI(EXT_PORT_TAG, "[%d:%d] Downstream device removed while enabled", ext_port->constant.parent_dev_addr,
                   ext_port->constant.port_num);
          ext_port->dev_state = PORT_DEV_NOT_PRESENT;
          port_event(ext_port, EXT_PORT_DISCONNECTED);
          ext_port->flags.waiting_recycle = 1;
          ...
          // Error state
          ESP_LOGE(EXT_PORT_TAG, "[%d:%d] Enabled, but doesn't have a device", ext_port->constant.parent_dev_addr,
                   ext_port->constant.port_num);
          new_state = USB_PORT_STATE_DISCONNECTED;
        }
      }
...
static void handle_port(ext_port_t *ext_port) {
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] change=0x%04x, status=0x%04x, state=%d", ext_port->constant.parent_dev_addr,
           ext_port->constant.port_num, ext_port->status.wPortChange.val, ext_port->status.wPortStatus.val,
           ext_port->state);
...
static void handle_recycle(ext_port_t *ext_port) {
  ...
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Port recycle (state=%d, dev=%d)", ext_port->constant.parent_dev_addr,
           ext_port->constant.port_num, ext_port->state, ext_port->dev_state);
...
static void handle_disable(ext_port_t *ext_port) {
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Disable (state=%d, dev=%d)", ext_port->constant.parent_dev_addr,
           ext_port->constant.port_num, ext_port->state, ext_port->dev_state);
  ...
  if (ext_port->state == USB_PORT_STATE_ENABLED) {
    if (port_has_connection(ext_port)) {
      ESP_LOGE(EXT_PORT_TAG, "[%d:%d] Port disabled, reset attempts=%d", ext_port->constant.parent_dev_addr,
               ext_port->constant.port_num, ext_port->dev_reset_attempts);
...
static esp_err_t port_new(void *port_cfg, void **port_hdl) {
  ...
  esp_err_t ret =
      port_alloc(config->context, config->parent_dev_hdl, config->parent_port_num, config->port_power_delay_ms, &port);
...
static esp_err_t port_reset(void *port_hdl) {
  ...
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Port reset request", ext_port->constant.parent_dev_addr, ext_port->constant.port_num);
...
static esp_err_t port_active(void *port_hdl) {
  ...
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Port has an enumerated device", ext_port->constant.parent_dev_addr,
           ext_port->constant.port_num);
...
static esp_err_t port_disable(void *port_hdl) {
  ...
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Disable", ext_port->constant.parent_dev_addr, ext_port->constant.port_num);
...
static esp_err_t port_delete(void *port_hdl) {
  ...
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Freeing", ext_port->constant.parent_dev_addr, ext_port->constant.port_num);
...
static esp_err_t port_gone(void *port_hdl) {
  ...
  ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Port is gone (state=%d, dev=%d)", ext_port->constant.parent_dev_addr,
           ext_port->constant.port_num, ext_port->state, ext_port->dev_state);
...
esp_err_t ext_port_process(void) {
  ...
  while (action_flags) {
    ...
    ESP_LOGD(EXT_PORT_TAG, "[%d:%d] Processing actions 0x%" PRIx32 "", ext_port->constant.parent_dev_addr,
             ext_port->constant.port_num, action_flags);
    ...
    if (action_flags & PORT_ACTION_GET_STATUS) {
      ...
      if (ext_port->state != USB_PORT_STATE_RESETTING) {
         ...
         */
        assert(ext_port->state == USB_PORT_STATE_DISCONNECTED || ext_port->state == USB_PORT_STATE_ENABLED);
        ext_port->state = USB_PORT_STATE_RESETTING;
      }
      port_set_feature(ext_port, USB_FEATURE_PORT_RESET);
    }
