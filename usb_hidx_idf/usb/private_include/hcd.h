...
typedef enum {
  ...
  HCD_PORT_STATE_DISABLED,  /**< A device has connected to the port but has not been reset. SOF/keep alive are not being
                               sent */
  ...
  HCD_PORT_STATE_RECOVERY,  /**< Port needs to be recovered from a fatal error (port error, overcurrent, or sudden
                               disconnection) */
} hcd_port_state_t;
...
typedef enum {
  ...
  HCD_PORT_CMD_POWER_OFF, /**< Power OFF the port. If the port is enabled, this will cause a
                             HCD_PORT_EVENT_SUDDEN_DISCONN event. If the port is disabled, this will cause a
                             HCD_PORT_EVENT_DISCONNECTION event. */
 ...
 */
typedef enum {
  HCD_PIPE_CMD_HALT,  /**< Halt an active pipe. The currently executing URB will be canceled. Enqueued URBs are left
                         untouched */
 ...
 */
typedef bool (*hcd_port_callback_t)(hcd_port_handle_t port_hdl, hcd_port_event_t port_event, void *user_arg,
                                    bool in_isr);
 ...
 */
typedef bool (*hcd_pipe_callback_t)(hcd_pipe_handle_t pipe_hdl, hcd_pipe_event_t pipe_event, void *user_arg,
                                    bool in_isr);
...
typedef struct {
  ...
  unsigned peripheral_idx;      /**< Index of USB peripheral this port belongs to. Index numbers are defined in
                                   USB_DWC_LL_GET_HW */
} hcd_port_config_t;
...
typedef struct {
  ...
  uint8_t tt_hub_addr;          /**< Address of the high-speed hub providing TT, or 0 */
  uint8_t tt_port_num;          /**< Downstream port on the TT hub, or 0 */
  bool tt_enabled;              /**< Enable DWC split transactions for this pipe */
} hcd_pipe_config_t;
