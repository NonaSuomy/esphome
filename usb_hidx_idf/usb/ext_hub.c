 ...
 */
typedef enum {
  EXT_HUB_STATE_ATTACHED,   /**< Device attached, but not state is unknown (no: Hub Descriptor, Device status and Hub
                               status) */
  EXT_HUB_STATE_CONFIGURED, /**< Device attached and configured (has Hub Descriptor, Device status and Hub status were
                               requested )*/
 ...
 * @brief Device stages
 *
 * During the lifecycle, Hub requires different actions. To implement interaction with external Hub the FSM, based on
 * these stages is using.
 *
 * Entry:
 * - Every new attached external Hub should start from EXT_HUB_STAGE_GET_HUB_DESCRIPTOR. Without Fetching Hub
 * Descriptor, device is not configured and doesn't have any ports.
 * - After handling the response during EXT_HUB_STAGE_CHECK_HUB_DESCRIPTOR, the External Hub Driver configures the
 * Device according to the data from Hub Descriptor.
 * - After completion of any stage, the Device does back to the IDLE stage and waits for another request. Source of the
 * request could be: EP1 INT callback (the Hub or Ports changes) or external call.
...
typedef enum {
  ...
  EXT_HUB_STAGE_GET_DEVICE_STATUS,    /**< Device requests Device Status. For more details, refer to 9.4.5 Get Status of
                                         usb_20 */
  EXT_HUB_STAGE_CHECK_DEVICE_STATUS,  /**< Device received the Device Status and required its' handling */
  EXT_HUB_STAGE_GET_HUB_DESCRIPTOR,   /**< Device requests Hub Descriptor. For more details, refer to 11.24.2.5 Get Hub
                                         Descriptor of usb_20 */
  EXT_HUB_STAGE_CHECK_HUB_DESCRIPTOR, /**< Device received the Hub Descriptor and requires its' handling  */
  EXT_HUB_STAGE_GET_HUB_STATUS, /**< Device requests Hub Status. For more details, refer to 11.24.2.6 Get Hub Status of
                                   usb_20 */
  ...
  EXT_HUB_STAGE_CHECK_PORT_FEATURE, /**< Device completed the Port Feature class-specific request (Set Feature or Clear
                                       Feature). For more details, refer to 11.24.2 Class-specific Requests of usb_20 */
  EXT_HUB_STAGE_CHECK_PORT_STATUS,  /**< Device completed the Port Get Status class-specific request. For more details,
...
const char *const ext_hub_stage_strings[] = {
    "IDLE",           "GET_DEVICE_STATUS", "CHECK_DEVICE_STATUS", "GET_HUB_DESCRIPTOR",  "CHECK_HUB_DESCRIPTOR",
    "GET_HUB_STATUS", "CHECK_HUB_STATUS",  "PORT_FEATURE",        "PORT_STATUS_REQUEST", "FAILURE"};
...
struct ext_hub_s {
  ...
  struct {
    ...
    uint8_t maxchild; /**< Amount of allocated ports. Could be 0 for some Hubs. Is increased when new port is added and
                         decreased when port has been freed. */
    ext_hub_state_t state; /**< Device's state */
  } single_thread; /**< Single thread members don't require a critical section, as long as they are never accessed from
                      multiple threads */
  ...
  switch (ep_event) {
    case USBH_EP_EVENT_URB_DONE: {
      ...
      case USBH_EP_EVENT_ERROR_OVERFLOW:
        // EP1's pipe has encountered an error. We need to retire all URBs, dequeue them, then make the pipe active
        // again
        action_flags = DEV_ACTION_EP1_FLUSH | DEV_ACTION_EP1_DEQUEUE | DEV_ACTION_EP1_CLEAR;
        if (in_isr) {
...
static void interrupt_transfer_complete_cb(usb_transfer_t *intr_xfer) {
  ...
  switch (intr_xfer->status) {
    ...
    default:
      // Any other error
      ESP_LOGE(EXT_HUB_TAG, "[%d] Interrupt transfer failed, status %d", ext_hub_dev->constant.dev_addr,
               intr_xfer->status);
...
static void device_error(ext_hub_dev_t *ext_hub_dev) {
  ...
  call_proc_req_cb = _device_set_actions(ext_hub_dev, DEV_ACTION_ERROR | DEV_ACTION_RELEASE);
  EXT_HUB_EXIT_CRITICAL();
...
static esp_err_t device_port_new(ext_hub_dev_t *ext_hub_dev, uint8_t port_idx) {
  ...
  esp_err_t ret =
      p_ext_hub_driver->constant.port_driver->new (&port_config, (void **) &ext_hub_dev->constant.ports[port_idx]);
  if (ret != ESP_OK) {
    ESP_LOGE(EXT_HUB_TAG, "[%d:%d] Port allocation error: %s", ext_hub_dev->constant.dev_addr, port_idx + 1,
             esp_err_to_name(ret));
    goto fail;
  }
...
static esp_err_t device_port_free(ext_hub_dev_t *ext_hub_dev, uint8_t port_idx) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(EXT_HUB_TAG, "[%d:%d] Unable to free port: %s", ext_hub_dev->constant.dev_addr, port_idx + 1,
             esp_err_to_name(ret));
    goto exit;
  }
...
static esp_err_t device_release_port(ext_hub_dev_t *ext_hub_dev, uint8_t port_idx) {
  ...
  if (ret == ESP_OK) {
    ...
    ESP_LOGE(EXT_HUB_TAG, "[%d:%d] Unable to mark port as gone: %s", ext_hub_dev->constant.dev_addr, port_idx + 1,
             esp_err_to_name(ret));
    return ret;
  }
...
static esp_err_t device_alloc_desc(ext_hub_dev_t *ext_hub_hdl, const usb_hub_descriptor_t *hub_desc) {
  ...
  usb_hub_descriptor_t *desc = heap_caps_malloc(
      hub_desc->bDescLength, MALLOC_CAP_DEFAULT);  // Buffer to copy over full configuration descriptor (wTotalLength)
  if (desc == NULL) {
...
static esp_err_t device_alloc(device_config_t *config, ext_hub_dev_t **ext_hub_dev) {
  ...
  if (hub_dev == NULL) {
    ESP_LOGE(EXT_HUB_TAG, "[%d] Unable to allocate device", config->dev_addr);
    ret = ESP_ERR_NO_MEM;
    goto fail;
  }
  ...
  if (ctrl_urb == NULL) {
    ESP_LOGE(EXT_HUB_TAG, "[%d] Unable to allocate Control URB", config->dev_addr);
    ret = ESP_ERR_NO_MEM;
    goto ctrl_urb_fail;
  }

  if (config->ep_in_desc->wMaxPacketSize > EXT_HUB_MAX_STATUS_BYTES_SIZE) {
    ESP_LOGE(EXT_HUB_TAG, "[%d] wMaxPacketSize=%d is not supported", config->dev_addr,
  ...
  if (in_urb == NULL) {
    ESP_LOGE(EXT_HUB_TAG, "[%d] Unable to allocate Interrupt URB", config->dev_addr);
    ret = ESP_ERR_NO_MEM;
    goto in_urb_fail;
  }
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(EXT_HUB_TAG, "[%d] Interrupt EP allocation failure: %s", config->dev_addr, esp_err_to_name(ret));
    goto ep_fail;
  }
...
static esp_err_t device_configure(ext_hub_dev_t *ext_hub_dev) {
  ...
  ESP_LOGD(EXT_HUB_TAG, "[%d] Device configure (iface %d)", ext_hub_dev->constant.dev_addr,
  ...
  ESP_LOGD(EXT_HUB_TAG, "\t%d external port%s", ext_hub_dev->constant.hub_desc->bNbrPorts,
  ...
  ext_hub_dev->constant.ports =
      heap_caps_calloc(ext_hub_dev->constant.hub_desc->bNbrPorts, sizeof(ext_port_hdl_t), MALLOC_CAP_DEFAULT);
...
static void handle_error(ext_hub_dev_t *ext_hub_dev) {
  ESP_LOGE(EXT_HUB_TAG, "[%d] Device is not working properly, wait device removal", ext_hub_dev->constant.dev_addr);
...
static bool handle_hub_descriptor(ext_hub_dev_t *ext_hub_dev) {
  ...
  const usb_hub_descriptor_t *hub_desc =
      (const usb_hub_descriptor_t *) (ctrl_xfer->data_buffer + sizeof(usb_setup_packet_t));
...
static bool handle_device_status(ext_hub_dev_t *ext_hub_dev) {
  usb_transfer_t *ctrl_xfer = &ext_hub_dev->constant.ctrl_urb->transfer;
  const usb_device_status_t *dev_status =
      (const usb_device_status_t *) (ctrl_xfer->data_buffer + sizeof(usb_setup_packet_t));
...
static void handle_port_status(ext_hub_dev_t *ext_hub_dev) {
  ...
  uint8_t port_num = USB_SETUP_PACKET_GET_PORT((usb_setup_packet_t *) ctrl_xfer->data_buffer);
  uint8_t port_idx = port_num - 1;
  const usb_port_status_t *new_status =
      (const usb_port_status_t *) (ctrl_xfer->data_buffer + sizeof(usb_setup_packet_t));
...
static bool device_control_response_handling(ext_hub_dev_t *ext_hub_dev) {
  ...
  if (ctrl_xfer->status != USB_TRANSFER_STATUS_COMPLETED) {
    ESP_LOGE(EXT_HUB_TAG, "[%d] Control request bad transfer status %d", ext_hub_dev->constant.dev_addr,
             ctrl_xfer->status);
    return stage_pass;
  }
...
static bool device_set_next_stage(ext_hub_dev_t *ext_hub_dev, bool last_stage_pass) {
  ...
  if (last_stage_pass) {
    ...
    if (last_stage == EXT_HUB_STAGE_GET_DEVICE_STATUS || last_stage == EXT_HUB_STAGE_GET_HUB_DESCRIPTOR ||
...
static esp_err_t find_first_intf_desc(const usb_config_desc_t *config_desc, device_config_t *hub_config) {
  ...
  const usb_intf_desc_t *next_intf_desc = (const usb_intf_desc_t *) usb_parse_next_descriptor_of_type(
      (const usb_standard_desc_t *) config_desc, config_desc->wTotalLength, USB_B_DESCRIPTOR_TYPE_INTERFACE, &offset);
  ...
  while (next_intf_desc != NULL) {
    ...
    if (USB_CLASS_HUB == next_intf_desc->bInterfaceClass) {
      ...
      if (!USB_EP_DESC_GET_EP_DIR(ep_in_desc) || (USB_EP_DESC_GET_XFERTYPE(ep_in_desc) != USB_TRANSFER_TYPE_INTR)) {
  ...
  next_iface:
    next_intf_desc = (const usb_intf_desc_t *) usb_parse_next_descriptor_of_type(
        (const usb_standard_desc_t *) next_intf_desc, config_desc->wTotalLength, USB_B_DESCRIPTOR_TYPE_INTERFACE,
        &offset);
  }
...
esp_err_t ext_hub_dev_gone(uint8_t dev_addr) {
  ...
  if (ext_hub_dev->dynamic.flags.waiting_release || ext_hub_dev->dynamic.flags.waiting_children) {
    EXT_HUB_EXIT_CRITICAL();
  ...
  if ((ext_hub_dev->single_thread.maxchild == 0) && !ext_hub_dev->dynamic.flags.in_pending_list) {
    ...
    ext_hub_dev->dynamic.flags.waiting_free = 1;
    ext_hub_dev->dynamic.flags.waiting_release = 1;
    call_proc_req_cb = _device_set_actions(ext_hub_dev, DEV_ACTION_RELEASE | DEV_ACTION_FREE);
  } else {
...
esp_err_t ext_hub_process(void) {
  ...
  while (!TAILQ_EMPTY(&p_ext_hub_driver->dynamic.ext_hubs_pending_tailq)) {
    ...
    if (action_flags & DEV_ACTION_REQ || action_flags & DEV_ACTION_EP0_COMPLETE) {
...
esp_err_t ext_hub_port_recycle(ext_hub_handle_t ext_hub_hdl, uint8_t port_num) {
  ...
  if (release_port) {
    ...
    if (ret != ESP_OK) {
      ESP_LOGE(EXT_HUB_TAG, "[%d:%d] Unable to recycle the port: %s", ext_hub_dev->constant.dev_addr, port_num,
               esp_err_to_name(ret));
    }
  }
  return ret;
...
esp_err_t ext_hub_port_disable(ext_hub_handle_t ext_hub_hdl, uint8_t port_num) {
  ...
  EXT_HUB_CHECK(p_ext_hub_driver->constant.port_driver != NULL, ESP_ERR_NOT_SUPPORTED);
  EXT_HUB_CHECK(ext_hub_dev->single_thread.state == EXT_HUB_STATE_CONFIGURED ||
                    ext_hub_dev->single_thread.state == EXT_HUB_STATE_SUSPENDED,
                ESP_ERR_INVALID_STATE);
...
static esp_err_t ext_hub_control_request(ext_hub_dev_t *ext_hub_dev, uint8_t port1, uint8_t request, uint8_t feature) {
  ...
  ret = usbh_dev_submit_ctrl_urb(ext_hub_dev->constant.dev_hdl, ext_hub_dev->constant.ctrl_urb);
  if (ret != ESP_OK) {
    ESP_LOGE(EXT_HUB_TAG, "Request %X, port %d, feature %d: failed to submit ctrl urb: %s", request, port1, feature,
             esp_err_to_name(ret));
    device_error(ext_hub_dev);
  }
  return ret;
