...
#if __has_include("soc/usb_periph.h")
...
#define RESUME_RECOVERY_MS \
  20  // Resume recovery of at least 10ms. Make it 20 ms to be safe. This will include the 3 LS bit times of the EOP
...
#define XFER_LIST_LEN_ISOC \
  64  // Implement longer ISOC transfer list to give us enough space for additional timing margin
#define XFER_LIST_ISOC_MARGIN \
  3                          // The 1st ISOC transfer is scheduled 3 (micro)frames later so we have enough timing margin
#define LS_CTRL_RETRY_MAX 3  // Bounded recovery for transient P4 PRE/LS launch errors

// The P4 HS-controller FS/LS fallback uses the UTMI 30 MHz clock selected by
// the upstream forced-FS workaround.  The standard 48 MHz FSLS setting is
// for the native FS PHY and causes the HS/UTMI hub link itself to fail.
#define P4_FSLS_UTMI_CLOCK_SEL 0
#define P4_FSLS_UTMI_FRAME_INTERVAL 29999
...
struct pipe_obj {
  ...
  union {
    struct {
      ...
      uint32_t
          rd_idx : 1;  // Index of the current buffer in-flight. Bit width must allow NUM_BUFFERS to wrap automatically
  ...
  uint8_t tt_port_num;
  bool tt_enabled;
  uint8_t tt_split_phase;  // 0=idle, 1=SSPLIT, 2=CSPLIT
  bool tt_retry_pending;
  // A P4 FS/LS-only HS root can report XCS_XACT_ERR while the PRE/LS
  // transaction is being launched. Retry only the current EP0 stage a
  // bounded number of times before returning the error to USBH.
  uint8_t ls_ctrl_retry_count;
  // The first PRE-qualified transfer is submitted immediately after the
  // hub port reset. Give the FS/LS scheduler one complete frame boundary
  // before that launch; later control stages are already separated by
  // _buffer_check_done().
  bool ls_first_launch_pending;
  // Port related
  ...
  volatile TaskHandle_t
      task_waiting_pipe_notif;  // Task handle used for internal pipe events. Set by waiter, cleared by notifier
  union {
    struct {
...
struct port_obj {
  ...
  volatile TaskHandle_t
      task_waiting_port_notif;  // Task handle used for internal port events. Set by waiter, cleared by notifier
  union {
    struct {
...
// Sticky diagnostic flag: a device or endpoint was refused because all DWC
// host channels were already allocated.  The USB host stack has no public
// resource-status API, so expose this small read-only diagnostic to HIDX.
static bool s_channel_allocation_failed = false;
...
static inline void cache_sync_xfer_descriptor_list(dma_buffer_block_t *buffer, bool mem_to_cache) {
  esp_err_t ret = esp_cache_msync(buffer->xfer_desc_list, buffer->xfer_desc_list_len_bytes,
                                  mem_to_cache ? ESP_CACHE_MSYNC_FLAG_DIR_M2C : 0);
  assert(ret == ESP_OK);
}
 ...
 *
 * @note Here we also accept UNALIGNED data, for cases where the class drivers force overwrite the allocated data
 * buffers
 *
...
/**
 * @brief Wait until the next USB frame before continuing an LS control transfer
 *        routed through an FS hub.
 *
 * The P4 DWC emits the FS PRE packet automatically when HCCHAR.LSpdDev is set.
 * Its FS/LS-only mode cannot safely issue two PRE-qualified transactions in
 * the same frame, however.  A fixed delay is not sufficient here because a
 * control-stage completion can occur immediately before or immediately after
 * the frame counter rolls over.  Wait for the actual HFNUM.FRNUM transition
 * instead, with a bounded fallback so an abnormal stopped frame clock cannot
 * wedge the ISR forever.
 */
static inline void _buffer_wait_next_preamble_frame(pipe_t *pipe) {
  const uint32_t start_frame = usb_dwc_ll_hfnum_get_frame_num(pipe->port->hal->dev);

  // 10 us polling keeps the wait short while allowing the FS frame counter
  // to advance.  The 3 ms bound is deliberately conservative: a normal FS
  // frame is 1 ms, but the host may be servicing this from an ISR near the
  // frame boundary.
  for (int waited_us = 0; waited_us < 3000; waited_us += 10) {
    if (usb_dwc_ll_hfnum_get_frame_num(pipe->port->hal->dev) != start_frame) {
      return;
    }
    esp_rom_delay_us(10);
  }
...
static inline bool _buffer_check_done(pipe_t *pipe) {
  ...
  // The HW can't handle two transactions with preamble in one frame.  Wait
  // for a real frame transition rather than assuming a 1 ms delay is enough.
  if (pipe->ep_char.ls_via_fs_hub) {
    _buffer_wait_next_preamble_frame(pipe);
  }
...
// A TT split is a two-part transaction.  The DWC host must not launch the
// complete split in the same microframe as the start split.  The stock HCD
// re-arms the channel immediately, which can leave an FS device behind a HS
// hub waiting forever on EP0.  Select the next USB frame before each deferred
// split attempt.  oddfrm is also honored for non-periodic channels by the
// ESP32-P4 DWC implementation.
static inline void _tt_schedule_next_frame(pipe_t *pipe) {
#if CONFIG_IDF_TARGET_ESP32P4
  uint32_t frame = usb_dwc_ll_hfnum_get_frame_num(pipe->port->hal->dev);
  // TT control transfers are non-periodic.  Do not write SCHED_INFO here:
  // on the P4 that field aliases the low HCTSIZ transfer-length bits for
  // non-periodic channels and changes an EP0 transfer into a 0xff-byte
  // request.  ODDFRM is sufficient to defer the split to the next frame.
  pipe->chan_obj->regs->hcchar_reg.oddfrm = (frame + 1) & 1;
#else
  (void) pipe;
#endif
}

static inline void _tt_schedule_current_frame(pipe_t *pipe) {
#if CONFIG_IDF_TARGET_ESP32P4
  uint32_t frame = usb_dwc_ll_hfnum_get_frame_num(pipe->port->hal->dev);
  pipe->chan_obj->regs->hcchar_reg.oddfrm = frame & 1;
#else
  (void) pipe;
#endif
}

static inline void _tt_log_arm(pipe_t *pipe, const char *phase) {
#ifdef USB_DWC_TT_DEBUG
#if CONFIG_IDF_TARGET_ESP32P4
  volatile usb_dwc_host_chan_regs_t *regs = pipe->chan_obj->regs;
  ESP_EARLY_LOGE(HCD_DWC_TAG,
                 "TT arm %s ch=%u HCCHAR=0x%08" PRIx32 " HCTSIZ=0x%08" PRIx32 " HCSPLT=0x%08" PRIx32
                 " HCDMA=0x%08" PRIx32,
                 phase, pipe->chan_obj->flags.chan_idx, regs->hcchar_reg.val, regs->hctsiz_reg.val,
                 regs->hcsplt_reg.val, regs->hcdma_reg.val);
#else
  (void) pipe;
  (void) phase;
#endif
#else
  // This helper is called from the transfer path immediately before a
  // split is armed.  Logging here at error level can run once per
  // microframe and starve the USB task, so low-level TT tracing is opt-in.
  (void) pipe;
  (void) phase;
#endif
}

static inline void _tt_set_csplit(pipe_t *pipe) {
#if CONFIG_IDF_TARGET_ESP32P4
  volatile usb_dwc_host_chan_regs_t *regs = pipe->chan_obj->regs;
  // For control, bulk, and interrupt transfers the split covers the whole
  // transaction.  XACTPOS=ALL (0) is the DWC encoding for a complete
  // non-isochronous split.  BEGIN/MID/END are only used to divide an
  // isochronous payload across multiple split transactions.
  regs->hcsplt_reg.xactpos = 0;
  regs->hcsplt_reg.compsplt = 1;
#else
  (void) pipe;
#endif
}

static inline void _tt_set_ssplit(pipe_t *pipe) {
#if CONFIG_IDF_TARGET_ESP32P4
  volatile usb_dwc_host_chan_regs_t *regs = pipe->chan_obj->regs;
  // For control, bulk, and interrupt transfers the split covers the whole
  // transaction.  XACTPOS=ALL (0) is the DWC encoding for a complete
  // non-isochronous split.
  regs->hcsplt_reg.xactpos = 0;
  regs->hcsplt_reg.compsplt = 0;
#else
  (void) pipe;
#endif
}

static inline void _tt_chan_activate(pipe_t *pipe, void *qtd_list, int qtd_len, int start_idx) {
#if CONFIG_IDF_TARGET_ESP32P4
  usb_dwc_hal_chan_t *chan = pipe->chan_obj;
  usb_dwc_dev_t *dev = pipe->port->hal->dev;
  usb_dwc_ll_hctsiz_set_dopng(chan->regs, false);
  usb_dwc_ll_hcdma_set_qtd_list_addr(chan->regs, qtd_list, start_idx);
  usb_dwc_ll_hctsiz_set_qtd_list_len(chan->regs, qtd_len);
  usb_dwc_ll_hcchar_enable_chan(chan->regs);
  chan->flags.active = 1;
#ifdef USB_DWC_TT_DEBUG
  ESP_EARLY_LOGE(HCD_DWC_TAG,
                 "TT activate ch=%u phase=%u HCCHAR=0x%08" PRIx32 " HCINT=0x%08" PRIx32 " HCINTMSK=0x%08" PRIx32
                 " HAINT=0x%08" PRIx32 " GINTSTS=0x%08" PRIx32 " GINTMSK=0x%08" PRIx32 " HFNUM=0x%08" PRIx32,
                 chan->flags.chan_idx, pipe->tt_split_phase, chan->regs->hcchar_reg.val, chan->regs->hcint_reg.val,
                 chan->regs->hcintmsk_reg.val, dev->haint_reg.val, dev->gintsts_reg.val, dev->gintmsk_reg.val,
                 dev->hfnum_reg.val);
#else
  (void) dev;
#endif
#else
  usb_dwc_hal_chan_activate(pipe->chan_obj, qtd_list, qtd_len, start_idx);
#endif
}
 ...
 * @param pipe_event Pipe event that caused the buffer to be complete. Use HCD_PIPE_EVENT_NONE for halt request of
 * disconnections
 * @param canceled Whether the buffer was done due to a canceled (i.e., halt request). Must set pipe_event to
 * HCD_PIPE_EVENT_NONE
 */
 ...
 *
 * @param pipe Pipe object
 * @param stop_idx (For INTR pipes only) The index of the descriptor that follows the last descriptor of the URB. Set to
 * 0 otherwise
 */
...
static usb_speed_t get_usb_port_speed(usb_dwc_speed_t priv) {
  switch (priv) {
    case USB_DWC_SPEED_LOW:
      return USB_SPEED_LOW;
    case USB_DWC_SPEED_FULL:
      return USB_SPEED_FULL;
    case USB_DWC_SPEED_HIGH:
...
#if CONFIG_IDF_TARGET_ESP32P4
// The P4 HS controller can run its UTMI PHY in FS/LS-only host mode. This is
// required for the hub workaround because the raw HPRT speed can remain HS
// even after the controller has been deliberately restricted to FS/LS. The
// mode must therefore be programmed unconditionally before every root reset.
static inline bool _p4_is_hs_controller(const port_t *port) { return port->hal->dev == &USB_DWC_HS; }

static inline void _p4_hs_fsls_host_init(port_t *port) {
  if (_p4_is_hs_controller(port)) {
    usb_dwc_dev_t *dev = port->hal->dev;
    // The value is intentionally programmed before the root-port reset.
    // The P4 HS core must also have descriptor DMA enabled before the
    // first reset; otherwise the root port can detect the connection but
    // never produce the enabled event used by the host stack.
    dev->hcfg_reg.descdma = 1;
    dev->hcfg_reg.fslssupp = 1;
    dev->hcfg_reg.fslspclksel = P4_FSLS_UTMI_CLOCK_SEL;
    usb_dwc_hfir_reg_t hfir;
    hfir.val = dev->hfir_reg.val;
    hfir.hfirrldctrl = 0;
    hfir.frint = P4_FSLS_UTMI_FRAME_INTERVAL;
    dev->hfir_reg.val = hfir.val;
    // P4's HS UTMI needs these receive-side controls enabled when the
    // HS root port is being used as an FS/LS-only host. Without them the
    // port can detect the attached hub/device but never complete reset.
    USB_UTMI.fc_06.det_fseop_en = 1;
    USB_UTMI.fc_06.pre_hphy_lsie = 1;
#ifdef USB_DWC_TT_DEBUG
    ESP_EARLY_LOGE(HCD_DWC_TAG, "P4 UTMI FC06=0x%08" PRIx32 " (LS_PAR=%u FS_EOP=%u PRE_HPHY_LSIE=%u)",
                   USB_UTMI.fc_06.val, USB_UTMI.fc_06.ls_par_en, USB_UTMI.fc_06.det_fseop_en,
                   USB_UTMI.fc_06.pre_hphy_lsie);
#endif
  }
}

static inline void _p4_hs_fsls_update_clock(port_t *port) {
  if (_p4_is_hs_controller(port)) {
    usb_dwc_dev_t *dev = port->hal->dev;
    // Keep the HS USB-A root link on the FS/LS host clock.  The P4 can
    // transiently report PrtSpd=LOW while an FS hub is completing reset;
    // switching HCFG to the 6 MHz LS clock at that point tears down the
    // root link before the hub can enumerate.  Downstream LS devices are
    // selected on their channel (LSpdDev + TT split), not by changing the
    // root controller clock.
    dev->hcfg_reg.descdma = 1;
    dev->hcfg_reg.perschedena = 0;
    dev->hcfg_reg.fslssupp = 1;
    dev->hcfg_reg.fslspclksel = P4_FSLS_UTMI_CLOCK_SEL;
    usb_dwc_hfir_reg_t hfir;
    hfir.val = dev->hfir_reg.val;
...
static usb_speed_t get_usb_port_speed_for_port(const port_t *port, usb_dwc_speed_t priv) {
#if CONFIG_IDF_TARGET_ESP32P4
  // In FS/LS-only mode the P4 may still report HPRT.PrtSpd=HS after link-up.
  if (_p4_is_hs_controller(port) && port->hal->dev->hcfg_reg.fslssupp && priv == USB_DWC_SPEED_HIGH) {
    priv = USB_DWC_SPEED_FULL;
  }
#else
  (void) port;
#endif
  return get_usb_port_speed(priv);
}

#if CONFIG_IDF_TARGET_ESP32P4
// Keep the P4 port snapshot in the project-local HCD while diagnosing hubs.
// This is intentionally emitted only at lifecycle/port-event boundaries; it
// must not run in the channel interrupt path, where it would flood the log.
static inline void _p4_log_port_snapshot(const port_t *port, const char *where) {
  const usb_dwc_dev_t *dev = port->hal->dev;
  ESP_EARLY_LOGE(HCD_DWC_TAG,
                 "%s state=%d HPRT=0x%08" PRIx32 " HCFG=0x%08" PRIx32 " HFNUM=0x%08" PRIx32 " HFIR=0x%08" PRIx32
                 " GINTSTS=0x%08" PRIx32 " GINTMSK=0x%08" PRIx32 " UTMI_FC06=0x%08" PRIx32,
                 where, port->state, dev->hprt_reg.val, dev->hcfg_reg.val, dev->hfnum_reg.val, dev->hfir_reg.val,
                 dev->gintsts_reg.val, dev->gintmsk_reg.val, USB_UTMI.fc_06.val);
}
#endif
...
#if CONFIG_IDF_TARGET_ESP32P4
  _p4_log_port_snapshot(port, "Port event");
#endif
...
#if CONFIG_IDF_TARGET_ESP32P4
      if (_p4_is_hs_controller(port)) {
        // Do not call the framework port-enable helper on the P4 HS
        // controller: older/newer ESP-IDF variants derive the root
        // clock from a transient HPRT.PrtSpd=LOW and switch to 6 MHz.
        // The HS USB-A root must remain on the FS/LS 30 MHz clock while
        // TT split pipes select LS per channel.
        _p4_hs_fsls_host_init(port);
        _p4_hs_fsls_update_clock(port);
      } else {
        usb_dwc_hal_port_enable(port->hal);
      }
#else
      usb_dwc_hal_port_enable(port->hal);  // Initialize remaining host port registers
#endif
      port->speed = get_usb_port_speed_for_port(port, usb_dwc_hal_port_get_conn_speed(port->hal));
 ...
 */
static void _tt_rearm_ctrl_qtd(dma_buffer_block_t *buffer) {
  int qtd_idx = buffer->flags.ctrl.cur_stg;
  // The DWC updates xfer_size in place when a split attempt completes.
  // Reusing that field makes the CSPLIT a zero-length transfer.  Rebuild
  // the descriptor from the immutable control URB instead.
  usb_transfer_t *transfer = &buffer->urb->transfer;
  int xfer_len;
  uint8_t *xfer_buf;
  if (qtd_idx == 0) {
    xfer_len = sizeof(usb_setup_packet_t);
    xfer_buf = transfer->data_buffer;
  } else if (qtd_idx == 1) {
    xfer_len = transfer->num_bytes - sizeof(usb_setup_packet_t);
    xfer_buf = transfer->data_buffer + sizeof(usb_setup_packet_t);
  } else {
    xfer_len = 0;
    xfer_buf = NULL;
  }
  uint32_t qtd_flags = USB_DWC_HAL_XFER_DESC_FLAG_HOC;
  if (qtd_idx == 0) {
    qtd_flags |= USB_DWC_HAL_XFER_DESC_FLAG_SETUP;
  } else if ((qtd_idx == 1 && buffer->flags.ctrl.data_stg_in) || (qtd_idx == 2 && !buffer->flags.ctrl.data_stg_in)) {
    qtd_flags |= USB_DWC_HAL_XFER_DESC_FLAG_IN;
  }
  usb_dwc_hal_xfer_desc_fill(buffer->xfer_desc_list, qtd_idx, xfer_buf, xfer_len, qtd_flags);
  // The descriptor list is cacheable on ESP32-P4.  This function is also
  // called from the ISR fast path, so explicitly push the rebuilt QTD
  // before re-enabling the DWC channel.
  CACHE_SYNC_XFER_DESCRIPTOR_LIST_C2M(buffer);
}

// Re-arm the active control stage after the DWC has halted it with a
// transaction error.  This is intentionally separate from the TT helper:
// the FS/LS-only P4 path does not use HCSPLT, but it still needs a fresh QTD
// after a failed PRE/LS attempt because descriptor DMA updates the QTD in
// place before raising XCS_XACT_ERR.
static inline void _ls_ctrl_retry(pipe_t *pipe, dma_buffer_block_t *buffer) {
  const int stage = buffer->flags.ctrl.cur_stg;
  bool next_dir_is_in;
  int next_pid;
  if (stage == 0) {
    next_dir_is_in = false;
    next_pid = 0;
  } else if (stage == 1) {
  ...
  _tt_rearm_ctrl_qtd(buffer);
  usb_dwc_hal_chan_set_dir(pipe->chan_obj, next_dir_is_in);
  usb_dwc_hal_chan_set_pid(pipe->chan_obj, next_pid);
  _buffer_wait_next_preamble_frame(pipe);
  usb_dwc_hal_chan_activate(pipe->chan_obj, buffer->xfer_desc_list, XFER_LIST_LEN_CTRL, stage);
}

static hcd_pipe_event_t _intr_hdlr_chan(pipe_t *pipe, usb_dwc_hal_chan_t *chan_obj, bool *yield) {
  // The generic HAL does not decode NYET/NAK. Capture HCINT before it is
  // cleared so split transactions can advance from SSPLIT to CSPLIT.
  uint32_t raw_chan_intrs = 0;
#if CONFIG_IDF_TARGET_ESP32P4
  if (pipe->tt_enabled) {
    raw_chan_intrs = chan_obj->regs->hcint_reg.val;
#ifdef USB_DWC_TT_DEBUG
    ESP_EARLY_LOGE(HCD_DWC_TAG, "TT ch=%u phase=%u HCINT=0x%08" PRIx32 " HCSPLT=0x%08" PRIx32, chan_obj->flags.chan_idx,
                   pipe->tt_split_phase, raw_chan_intrs, chan_obj->regs->hcsplt_reg.val);
#endif
    // ACK/NAK/NYET are enabled by the DWC split engine and can arrive
    // without CHHLTD.  The generic ESP-IDF decoder assumes every error
    // has CHHLTD and asserts, so consume these split-only events here.
    const uint32_t split_errors = USB_DWC_LL_INTR_CHAN_AHBERR | USB_DWC_LL_INTR_CHAN_STALL |
                                  USB_DWC_LL_INTR_CHAN_XACTERR | USB_DWC_LL_INTR_CHAN_BBLEER |
                                  USB_DWC_LL_INTR_CHAN_DATATGLERR | USB_DWC_LL_INTR_CHAN_BNAINTR |
                                  USB_DWC_LL_INTR_CHAN_XCS_XACT_ERR | USB_DWC_LL_INTR_CHAN_DESC_LS_ROLL;
    if (!(raw_chan_intrs & USB_DWC_LL_INTR_CHAN_CHHLTD) &&
        (raw_chan_intrs &
         (USB_DWC_LL_INTR_CHAN_ACK | USB_DWC_LL_INTR_CHAN_NAK | USB_DWC_LL_INTR_CHAN_NYET | split_errors))) {
      volatile usb_dwc_host_chan_regs_t *regs = chan_obj->regs;
      regs->hcint_reg.val = raw_chan_intrs;
      if (raw_chan_intrs & split_errors) {
        // Some ESP32-P4 revisions report an error before CHHLTD.
        // Defer the retry until the channel has actually halted;
        // immediate reactivation can storm the interrupt watchdog.
        pipe->tt_retry_pending = true;
        pipe->tt_split_phase = 1;
        _tt_set_ssplit(pipe);
        _tt_schedule_next_frame(pipe);
        regs->hcchar_reg.chdis = 1;
        chan_obj->flags.active = 1;
        return HCD_PIPE_EVENT_NONE;
      } else if (raw_chan_intrs & USB_DWC_LL_INTR_CHAN_NYET) {
        pipe->tt_split_phase = 2;
        _tt_set_csplit(pipe);
        _tt_schedule_current_frame(pipe);
      } else if (raw_chan_intrs & USB_DWC_LL_INTR_CHAN_NAK) {
        pipe->tt_split_phase = 1;
        _tt_set_ssplit(pipe);
        _tt_schedule_next_frame(pipe);
      } else if (pipe->tt_split_phase == 1) {
        pipe->tt_split_phase = 2;
        _tt_set_csplit(pipe);
        _tt_schedule_current_frame(pipe);
      }
      // ACK/NAK/NYET can be reported before CHHLTD on ESP32-P4.  In
      // that fast path the channel is re-enabled directly, so refresh
      // the control QTD here as well; otherwise the DWC sees the
      // post-transfer zero length left by the previous split.
      if (pipe->ep_char.type == USB_DWC_XFER_TYPE_CTRL) {
        dma_buffer_block_t *split_buffer = pipe->buffers[pipe->multi_buffer_control.rd_idx];
        _tt_rearm_ctrl_qtd(split_buffer);
      }
      regs->hcchar_reg.chdis = 0;
      regs->hcchar_reg.chena = 1;
      return HCD_PIPE_EVENT_NONE;
    }
  }
#endif
#if CONFIG_IDF_TARGET_ESP32P4
  if (pipe->ep_char.ls_via_fs_hub) {
    // Preserve HCINT for the diagnostic in the error path. The generic
    // HAL clears HCINT while decoding the event. Do not log here: this
    // path runs for every successful low-speed interrupt poll (typically
    // every 500 ms), and logging from the HCD interrupt path can starve
    // the LVGL/display task and make the screen appear to flash.
    raw_chan_intrs = chan_obj->regs->hcint_reg.val;
  }
#endif
  ...
  switch (chan_event) {
    case USB_DWC_HAL_CHAN_EVENT_CPLT: {
      if (pipe->tt_retry_pending) {
        dma_buffer_block_t *retry_buffer = pipe->buffers[pipe->multi_buffer_control.rd_idx];
        pipe->tt_retry_pending = false;
        if (pipe->ep_char.type == USB_DWC_XFER_TYPE_CTRL) {
          _tt_rearm_ctrl_qtd(retry_buffer);
        }
        pipe->tt_split_phase = 1;
        _tt_set_ssplit(pipe);
        int retry_start_idx = (pipe->ep_char.type == USB_DWC_XFER_TYPE_CTRL) ? retry_buffer->flags.ctrl.cur_stg : 0;
        chan_obj->regs->hcchar_reg.chdis = 0;
        _tt_chan_activate(pipe, retry_buffer->xfer_desc_list,
                          pipe->ep_char.type == USB_DWC_XFER_TYPE_CTRL ? XFER_LIST_LEN_CTRL : 1, retry_start_idx);
        break;
      }
      if (pipe->tt_enabled && pipe->tt_split_phase != 0) {
#if CONFIG_IDF_TARGET_ESP32P4
        volatile usb_dwc_host_chan_regs_t *regs = chan_obj->regs;
        bool nyet = (raw_chan_intrs & (1U << 6)) != 0;
        bool nak = (raw_chan_intrs & (1U << 4)) != 0;
        if (pipe->tt_split_phase == 1) {
          if (!nyet && !nak) {
            pipe->tt_split_phase = 2;
            _tt_set_csplit(pipe);
          } else {
          ...
          // The complete split must be issued in a later frame than
          // the start split.  Scheduling it in the current frame can
          // make the P4 DWC report XCS_XACT_ERR before the TT has
          // made the FS result available.
          _tt_schedule_next_frame(pipe);
          _tt_log_arm(pipe, "CSPLIT");
        } else if (nyet) {
          _tt_set_csplit(pipe);
          _tt_schedule_next_frame(pipe);
          _tt_log_arm(pipe, "CSPLIT-RETRY");
        } else {
          pipe->tt_split_phase = 0;
          goto split_complete;
        }
        dma_buffer_block_t *split_buffer = pipe->buffers[pipe->multi_buffer_control.rd_idx];
        int split_len = 1;
        if (pipe->ep_char.type == USB_DWC_XFER_TYPE_INTR) {
          split_len = split_buffer->flags.intr.zero_len_packet ? split_buffer->flags.intr.num_qtds + 1
                                                               : split_buffer->flags.intr.num_qtds;
        } else if (pipe->ep_char.type == USB_DWC_XFER_TYPE_CTRL) {
          split_len = XFER_LIST_LEN_CTRL;
        } else if (pipe->ep_char.type == USB_DWC_XFER_TYPE_BULK) {
          split_len = split_buffer->flags.bulk.zero_len_packet ? XFER_LIST_LEN_BULK : 1;
        }
        // The DWC QTD is consumed by the SSPLIT attempt (its ACTIVE bit
        // is cleared when the channel halts).  A CSPLIT must therefore
        // be given a freshly armed QTD.  Without this, the controller
        // restarts the channel with a NOT_EXECUTED descriptor and EP0
        // enumeration fails as a short device descriptor.
        if (pipe->ep_char.type == USB_DWC_XFER_TYPE_CTRL) {
          _tt_rearm_ctrl_qtd(split_buffer);
        }
        // A split control transfer must resume the same control stage that
        // generated the split.  Restarting at QTD 0 replays the SETUP
        // packet when the data/status stage is being completed, which
        // makes the device return an invalid/short descriptor.
        int split_start_idx = 0;
        if (pipe->ep_char.type == USB_DWC_XFER_TYPE_CTRL) {
          split_start_idx = split_buffer->flags.ctrl.cur_stg;
        }
        chan_obj->regs->hcchar_reg.chdis = 0;
        _tt_chan_activate(pipe, split_buffer->xfer_desc_list, split_len, split_start_idx);
        break;
#endif
      }
    split_complete:
      ...
      // First check if there is another buffer we can execute. But we only want to execute if there's still a valid
      // device
...
#ifdef USB_DWC_TT_DEBUG
        ESP_EARLY_LOGE(HCD_DWC_TAG,
                       "LS error detail ch=%u type=%u err=%u conn=%u retry=%u/%u HCINT=0x%08" PRIx32
                       " HCCHAR=0x%08" PRIx32 " HCTSIZ=0x%08" PRIx32,
                       chan_obj->flags.chan_idx, pipe->ep_char.type, chan_error, pipe->port->flags.conn_dev_ena,
                       pipe->ls_ctrl_retry_count, LS_CTRL_RETRY_MAX, raw_chan_intrs, chan_obj->regs->hcchar_reg.val,
                       chan_obj->regs->hctsiz_reg.val);
#endif
      }
#endif
      // On the P4 HS controller in FS/LS-only mode, an LS device behind an
      // FS hub can occasionally lose the PRE/LS launch at the first EP0
      // stage. Retry the same control stage on a later frame with a fresh
      // descriptor. Keep this bounded: a permanently unsupported PHY must
      // still fail cleanly instead of generating an interrupt storm.
      if (pipe->ep_char.ls_via_fs_hub && pipe->ep_char.type == USB_DWC_XFER_TYPE_CTRL &&
          chan_error == USB_DWC_HAL_CHAN_ERROR_XCS_XACT && pipe->ls_ctrl_retry_count < LS_CTRL_RETRY_MAX &&
          pipe->port->flags.conn_dev_ena) {
        dma_buffer_block_t *retry_buffer = pipe->buffers[pipe->multi_buffer_control.rd_idx];
        pipe->ls_ctrl_retry_count++;
#ifdef USB_DWC_TT_DEBUG
        ESP_EARLY_LOGE(HCD_DWC_TAG, "Retry LS control stage=%u attempt=%u/%u", retry_buffer->flags.ctrl.cur_stg,
                       pipe->ls_ctrl_retry_count, LS_CTRL_RETRY_MAX);
#endif
        _ls_ctrl_retry(pipe, retry_buffer);
        // usb_dwc_hal_chan_decode_intr() clears the HAL active flag when
        // it reports XCS_XACT.  Re-arm both sides of the channel state
        // before returning from the ISR, otherwise the DWC may accept
        // the register write but the next interrupt is not dispatched.
        chan_obj->regs->hcchar_reg.chdis = 0;
        chan_obj->regs->hcchar_reg.chena = 1;
        chan_obj->flags.active = 1;
        break;
      }
      // A split transaction can legitimately report a transient transaction
      // error while the TT is busy. Retry the same control stage as a new
      // SSPLIT instead of surfacing an URB error to USBH.
      if (pipe->tt_enabled && pipe->ep_char.type == USB_DWC_XFER_TYPE_CTRL &&
          chan_error == USB_DWC_HAL_CHAN_ERROR_XCS_XACT) {
        dma_buffer_block_t *retry_buffer = pipe->buffers[pipe->multi_buffer_control.rd_idx];
        _tt_rearm_ctrl_qtd(retry_buffer);
        // A transaction error on CSPLIT invalidates the TT transaction.
        // Restart with a new SSPLIT, as required by the DWC2 split state
        // machine; retrying CSPLIT alone cannot create a new TT request.
        pipe->tt_split_phase = 1;
        _tt_set_ssplit(pipe);
        _tt_schedule_next_frame(pipe);
        chan_obj->regs->hcchar_reg.chdis = 0;
        _tt_chan_activate(pipe, retry_buffer->xfer_desc_list, XFER_LIST_LEN_CTRL, retry_buffer->flags.ctrl.cur_stg);
        break;
      }
    ...
    case USB_DWC_HAL_CHAN_EVENT_HALT_REQ: {
      ...
      pipe->last_event = HCD_PIPE_EVENT_URB_DONE;
      event = pipe->last_event;
      // Halt request event is triggered when packet is successful completed. But just treat all halted transfers as
      // errors
      pipe->state = HCD_PIPE_STATE_HALTED;
      int stop_idx = usb_dwc_hal_chan_get_qtd_idx(chan_obj);
...
static void _calculate_fifo_from_bias(port_t *port, const usb_dwc_hal_context_t *hal) {
  ...
  port->fifo_config.rx_fifo_lines =
      fifo_size_lines - port->fifo_config.ptx_fifo_lines - port->fifo_config.nptx_fifo_lines;
  ...
  port->fifo_config.rx_fifo_lines =
      otg_dfifo_depth / 8 +
      2;  // 2 extra lines are allocated for status information. See USB-OTG Programming Guide, chapter 2.1.2.1
  port->fifo_config.nptx_fifo_lines = otg_dfifo_depth / 16;
  port->fifo_config.ptx_fifo_lines =
      fifo_size_lines - port->fifo_config.nptx_fifo_lines - port->fifo_config.rx_fifo_lines;
...
static port_t *port_obj_alloc(void) {
  ...
  void *frame_list = heap_caps_aligned_calloc(USB_DWC_FRAME_LIST_MEM_ALIGN, FRAME_LIST_LEN, sizeof(uint32_t),
                                              MALLOC_CAP_DMA | MALLOC_CAP_CACHE_ALIGNED | MALLOC_CAP_INTERNAL);
...
// Read the current DWC channel usage for the first installed root port.
// Returns false until the HCD has been installed.  The failure flag remains
// set until the HCD is reinstalled/reset, so an already-disabled downstream
// port remains visible to the user as a resource failure.
bool usb_dwc_hcd_get_channel_status(int *allocated, int *total, bool *exhausted) {
  bool found = false;
  HCD_ENTER_CRITICAL();
  if (s_hcd_obj != NULL) {
    for (int i = 0; i < NUM_PORTS; i++) {
      port_t *port = s_hcd_obj->port_obj[i];
      if (port != NULL && port->hal != NULL) {
        if (allocated != NULL) {
          *allocated = port->hal->channels.num_allocated;
        }
        if (total != NULL) {
          *total = (int) port->hal->constant_config.chan_num_total;
        }
        found = true;
        break;
      }
  ...
  if (exhausted != NULL) {
    *exhausted = s_channel_allocation_failed;
  }
  HCD_EXIT_CRITICAL();
  return found;
}
...
esp_err_t hcd_install(const hcd_config_t *config) {
  ...
  s_channel_allocation_failed = false;
  HCD_EXIT_CRITICAL();
  ...
  for (int i = 0; i < NUM_PORTS; i++) {
    if (BIT(i) & config->peripheral_map) {
      ...
      err_ret = esp_intr_alloc(
          irq_index,
          config->intr_flags |
              ESP_INTR_FLAG_INTRDISABLED,  // The interrupt must be disabled until the port is initialized
          intr_hdlr_main, (void *) p_hcd_obj_dmy->port_obj[i], &p_hcd_obj_dmy->port_obj[i]->isr_hdl);
      if (err_ret != ESP_OK) {
...
static inline bool _is_fifo_config_by_bias(const usb_dwc_hal_fifo_config_t *cfg) {
  return (cfg->rx_fifo_lines == 0 && cfg->nptx_fifo_lines == 0 && cfg->ptx_fifo_lines == 0);
}
...
#if CONFIG_IDF_TARGET_ESP32P4
    // Re-apply the mode selected from the current root-link speed after
    // the HPRT power write. This is a no-op for a direct LS/HS root.
    _p4_hs_fsls_host_init(port);
#endif
#if CONFIG_IDF_TARGET_ESP32P4
    _p4_log_port_snapshot(port, "Port power-on");
#endif
    ret = ESP_OK;
  } else {
...
static esp_err_t _port_cmd_reset(port_t *port) {
  ...
  */
...
#if CONFIG_IDF_TARGET_ESP32P4
  _p4_log_port_snapshot(port, "Reset begin");
#endif

  // Place the bus into the reset state. If the port was previously enabled, a disabled event will occur after this
#if CONFIG_IDF_TARGET_ESP32P4
  // The controller can clear FSLSSupp during a reset transition; re-apply
  // the speed-appropriate mode immediately before asserting reset.
  _p4_hs_fsls_host_init(port);
#endif
  usb_dwc_hal_port_toggle_reset(port->hal, true);
#if CONFIG_IDF_TARGET_ESP32P4
  _p4_log_port_snapshot(port, "Reset asserted");
#endif
  HCD_EXIT_CRITICAL();
...
#if CONFIG_IDF_TARGET_ESP32P4
  _p4_log_port_snapshot(port, "Reset released");
#endif
  HCD_EXIT_CRITICAL();
  ...
  ret = ESP_OK;
bailout:
  // A failed reset must leave the port recoverable. Returning an error while
  // leaving the state as RESETTING/DISABLED strands the hub state machine and
  // makes every later hot-plug fail at the root port.
  if (ret != ESP_OK) {
#if CONFIG_IDF_TARGET_ESP32P4
    ESP_EARLY_LOGE(HCD_DWC_TAG,
                   "Port reset failed: state=%d flags=0x%08" PRIx32 " HPRT=0x%08" PRIx32 " HCFG=0x%08" PRIx32,
                   port->state, port->flags.val, port->hal->dev->hprt_reg.val, port->hal->dev->hcfg_reg.val);
#endif
    port->flags.conn_dev_ena = 0;
    port->state = HCD_PORT_STATE_RECOVERY;
  }
...
esp_err_t hcd_port_init(int port_number, const hcd_port_config_t *port_config, hcd_port_handle_t *port_hdl) {
  ...
  // Port object memory and resources (such as the mutex) already be allocated. Just need to initialize necessary fields
  // only
...
#if CONFIG_IDF_TARGET_ESP32P4
  _p4_hs_fsls_host_init(port_obj);
#endif
#if CONFIG_IDF_TARGET_ESP32P4
#ifdef USB_DWC_TT_DEBUG
  ESP_EARLY_LOGE(HCD_DWC_TAG, "Project P4 dynamic HS/FS-LS host mode active on port=%d", port_number);
#endif
  _p4_log_port_snapshot(port_obj, "Port init");
#endif
  ...
  *port_hdl = (hcd_port_handle_t) port_obj;
  ...
  ESP_LOGD(HCD_DWC_TAG, "FIFO config lines: RX=%" PRIu32 ", PTX=%" PRIu32 ", NPTX=%" PRIu32,
           port_obj->fifo_config.rx_fifo_lines, port_obj->fifo_config.ptx_fifo_lines,
...
esp_err_t hcd_port_deinit(hcd_port_handle_t port_hdl) {
  ...
  HCD_ENTER_CRITICAL();
  HCD_CHECK_FROM_CRIT(s_hcd_obj != NULL && port->initialized && port->num_pipes_idle == 0 &&
                          port->num_pipes_queued == 0 &&
...
esp_err_t hcd_port_command(hcd_port_handle_t port_hdl, hcd_port_cmd_t command) {
  ...
  HCD_ENTER_CRITICAL();
  if (port->initialized &&
      !port->flags.event_pending) {  // Port events need to be handled first before issuing a command
...
esp_err_t hcd_port_get_speed(hcd_port_handle_t port_hdl, usb_speed_t *speed) {
  ...
  *speed = get_usb_port_speed_for_port(port, usb_dwc_hal_port_get_conn_speed(port->hal));
  HCD_EXIT_CRITICAL();
  return ESP_OK;
}
...
esp_err_t hcd_port_recover(hcd_port_handle_t port_hdl) {
  ...
  HCD_CHECK_FROM_CRIT(s_hcd_obj != NULL && port->initialized && port->state == HCD_PORT_STATE_RECOVERY &&
                          port->num_pipes_idle == 0 && port->num_pipes_queued == 0 && port->flags.val == 0 &&
                          port->task_waiting_port_notif == NULL,
                      ESP_ERR_INVALID_STATE);
...
static dma_buffer_block_t *buffer_block_alloc(usb_transfer_type_t type) {
  ...
  // Transfer descriptor list: Must be 512 aligned and DMA capable (USB-DWC requirement) and its size must be cache
  // aligned
  void *xfer_desc_list = heap_caps_aligned_calloc(USB_DWC_QTD_LIST_MEM_ALIGN,
                                                  desc_list_len * sizeof(usb_dwc_ll_dma_qtd_t), 1, XFER_DESC_LIST_CAPS);
  ...
  // Note for developers: We do not use heap_caps_get_allocated_size() because it is broken with
  // HEAP_POISONING=COMPREHENSIVE
...
static bool pipe_args_usb_compliance_verification(const hcd_pipe_config_t *pipe_config, usb_speed_t port_speed,
  ...
  if (pipe_config->dev_speed == USB_SPEED_LOW &&
      (type == USB_TRANSFER_TYPE_BULK || type == USB_TRANSFER_TYPE_ISOCHRONOUS)) {
...
static bool pipe_alloc_hcd_support_verification(usb_dwc_hal_context_t *hal, const usb_ep_desc_t *ep_desc) {
  ...
  if ((type == USB_TRANSFER_TYPE_INTR || type == USB_TRANSFER_TYPE_ISOCHRONOUS) && (ep_desc->bInterval == 0)) {
    ESP_LOGE(HCD_DWC_TAG, "bInterval value (%d) invalid for pipe type INTR/ISOC", ep_desc->bInterval);
    return false;
  }
  ...
  if (USB_EP_DESC_GET_MPS(ep_desc) > limit) {
    ESP_LOGE(HCD_DWC_TAG, "EP MPS (%d) exceeds supported limit (%d)", USB_EP_DESC_GET_MPS(ep_desc), limit);
    return false;
  }
...
static void pipe_set_ep_char(const hcd_pipe_config_t *pipe_config, usb_transfer_type_t type, bool is_default_pipe,
  ...
  // A low-speed device behind a full-speed hub needs a PREamble on every
  // transaction, including the default control pipe used for enumeration.
  // The stock condition excluded pipe_idx == 0, so the first EP0 request
  // (GET_DESCRIPTOR) was sent without LSPDDEV and failed before any
  // endpoint could be created.  Direct low-speed devices are unaffected
  // because their root port speed is LOW, not FULL.
  if (port_speed == USB_SPEED_FULL && pipe_config->dev_speed == USB_SPEED_LOW) {
    ESP_LOGW(HCD_DWC_TAG, "Low-speed device via FS hub; PREamble enabled");
...
static void pipe_set_tt(pipe_t *pipe) {
#if CONFIG_IDF_TARGET_ESP32P4
  if (pipe->tt_enabled && pipe->ep_char.type != USB_DWC_XFER_TYPE_ISOCHRONOUS) {
    volatile usb_dwc_host_chan_regs_t *regs = pipe->chan_obj->regs;
    regs->hcsplt_reg.prtaddr = pipe->tt_port_num;
    regs->hcsplt_reg.hubaddr = pipe->tt_hub_addr;
    // Non-isochronous control and bulk splits use XACTPOS=ALL (0).
    _tt_set_ssplit(pipe);
    regs->hcsplt_reg.spltena = 1;
  }
#else
  (void) pipe;
#endif
      ...
      if (pipe->ep_char.type == USB_DWC_XFER_TYPE_ISOCHRONOUS) {
        ...
        for (int pkt_idx = 0; pkt_idx < urb->transfer.num_isoc_packets; pkt_idx++) {
          ...
          urb->transfer.isoc_packet_desc[pkt_idx].status =
              (canceled) ? USB_TRANSFER_STATUS_CANCELED : USB_TRANSFER_STATUS_NO_DEVICE;
        }
      }
    }
...
esp_err_t hcd_pipe_alloc(hcd_port_handle_t port_hdl, const hcd_pipe_config_t *pipe_config,
  ...
  pipe->tt_hub_addr = pipe_config->tt_hub_addr;
  pipe->tt_port_num = pipe_config->tt_port_num;
  pipe->tt_enabled = pipe_config->tt_enabled && pipe_config->dev_speed != USB_SPEED_HIGH;
  // The ESP32-P4 HCD normally sets LSPDDEV only for a low-speed device
  // reached through a full-speed hub.  A high-speed hub's TT still needs
  // the channel to be programmed for the downstream low-speed signaling;
  // the upstream transaction is then carried as SSPLIT/CSPLIT.  Without
  // this bit the TT is addressed correctly but the downstream EP0 request
  // is sent with the wrong low-speed/preamble handling and returns STALL.
  if (pipe->tt_enabled && pipe_config->dev_speed == USB_SPEED_LOW) {
    pipe->ep_char.ls_via_fs_hub = 1;
  }

#if CONFIG_IDF_TARGET_ESP32P4
  // In the P4 HS-controller FS/LS-only mode, the root port reports FULL
  // speed even though a low-speed child may be reached through an FS hub.
  // Keep this explicit at allocation time so the default EP0 pipe cannot
  // lose the PREamble requirement during the speed transition.
  if (_p4_is_hs_controller(port) && port_speed == USB_SPEED_FULL && pipe_config->dev_speed == USB_SPEED_LOW) {
    pipe->ep_char.ls_via_fs_hub = 1;
  }
#endif
  pipe->ls_first_launch_pending = pipe->ep_char.ls_via_fs_hub;
  ...
  if (!chan_allocated) {
    s_channel_allocation_failed = true;
    HCD_EXIT_CRITICAL();
  ...
  if (pipe_config->dev_speed == USB_SPEED_LOW) {
#ifdef USB_DWC_TT_DEBUG
    ESP_EARLY_LOGE(HCD_DWC_TAG, "LS pipe idx=%d root_speed=%d preamble=%d HCCHAR=0x%08" PRIx32 " HCFG=0x%08" PRIx32,
                   pipe_idx, port_speed, pipe->ep_char.ls_via_fs_hub, pipe->chan_obj->regs->hcchar_reg.val,
                   port->hal->dev->hcfg_reg.val);
#endif
  }
#endif
  pipe_set_tt(pipe);
  if (pipe->tt_enabled) {
    // The P4 DWC uses the periodic frame list to honor ODDFRM for split
    // follow-up transactions, even when the USB endpoint is control or
    // bulk.  The USB transfer type remains unchanged in HCCHAR; this
    // only makes the host scheduler visit the TT channel at the selected
    // frame boundary.
    for (int frame = 0; frame < FRAME_LIST_LEN; frame++) {
      ((uint32_t *) port->frame_list)[frame] |= 1U << pipe->chan_obj->flags.chan_idx;
    }
    usb_dwc_ll_hcfg_en_perio_sched(port->hal->dev);
  }
  CACHE_SYNC_FRAME_LIST(port->frame_list);
...
esp_err_t hcd_pipe_free(hcd_pipe_handle_t pipe_hdl) {
  ...
  HCD_CHECK_FROM_CRIT(!pipe->multi_buffer_control.buffer_is_executing && !pipe->cs_flags.has_urb,
                      ESP_ERR_INVALID_STATE);
...
esp_err_t hcd_pipe_update_mps(hcd_pipe_handle_t pipe_hdl, int mps) {
  ...
  HCD_CHECK_FROM_CRIT(!pipe->cs_flags.pipe_cmd_processing && !pipe->cs_flags.has_urb, ESP_ERR_INVALID_STATE);
...
static inline void _buffer_fill_ctrl(dma_buffer_block_t *buffer, usb_transfer_t *transfer) {
  ...
  if (data_stg_skip) {
    ...
    // Fill data stage. Note that we still fill with transfer->num_bytes instead of setup_pkt->wLength as it's possible
    // to require more bytes than wLength
    usb_dwc_hal_xfer_desc_fill(buffer->xfer_desc_list, 1, transfer->data_buffer + sizeof(usb_setup_packet_t),
  ...
  usb_dwc_hal_xfer_desc_fill(
      buffer->xfer_desc_list, 2, NULL, 0,
...
static inline void _buffer_fill_intr(dma_buffer_block_t *buffer, usb_transfer_t *transfer, bool is_in, int mps) {
  ...
  assert((zero_len_packet)
             ? num_qtds + 1
             : num_qtds <= XFER_LIST_LEN_INTR);  // Check that the number of QTDs doesn't exceed the QTD list's length
  ...
  if (zero_len_packet) {
    ...
    usb_dwc_hal_xfer_desc_fill(buffer->xfer_desc_list, num_qtds - 1, &transfer->data_buffer[bytes_filled],
                               transfer->num_bytes - bytes_filled, xfer_desc_flags);
    ...
    usb_dwc_hal_xfer_desc_fill(buffer->xfer_desc_list, num_qtds - 1, &transfer->data_buffer[bytes_filled],
                               transfer->num_bytes - bytes_filled, xfer_desc_flags | USB_DWC_HAL_XFER_DESC_FLAG_HOC);
  }
...
static inline void IRAM_ATTR _buffer_fill_isoc(dma_buffer_block_t *buffer, usb_transfer_t *transfer, bool is_in,
                                               int mps, int interval, int start_idx) {
  assert(interval > 0);
  assert(__builtin_popcount(interval) ==
         1);  // Isochronous interval must be power of 2 according to USB2.0 specification
  int total_num_desc = transfer->num_isoc_packets * interval;
  assert(total_num_desc <=
         XFER_LIST_LEN_ISOC - XFER_LIST_ISOC_MARGIN);  // Some space in the qTD list is reserved for timing margin
...
static void IRAM_ATTR _buffer_fill(pipe_t *pipe) {
  ...
  switch (pipe->ep_char.type) {
    ...
    case USB_DWC_XFER_TYPE_ISOCHRONOUS: {
      uint16_t start_idx;
      // Interval in frames (FS) or microframes (HS). But it does not matter here, as each QTD represents one
      // transaction in a frame or microframe
      unsigned int interval = pipe->ep_char.periodic.interval;
      if (interval > XFER_LIST_LEN_ISOC) {
        // Each QTD in the list corresponds to one frame/microframe. Interval > Descriptor_list does not make sense
        // here.
        interval = XFER_LIST_LEN_ISOC;
      }
      if (pipe->multi_buffer_control.buffer_num_to_exec == 0) {
        // There are no more previously filled buffers to execute. We need to calculate a new start index based on HFNUM
        // and the pipe's schedule
        ...
        start_idx += XFER_LIST_ISOC_MARGIN;  // Start scheduling with a little delay. This will get us enough timing
                                             // margin so no transfer is skipped
...
static void IRAM_ATTR _buffer_exec(pipe_t *pipe) {
  assert(pipe->multi_buffer_control.rd_idx != pipe->multi_buffer_control.wr_idx ||
  ...
  pipe->ls_ctrl_retry_count = 0;

  // The P4 HS controller in FS/LS-only mode can lose the first PRE/LS
  // launch when it is submitted in the same frame as the hub-port reset
  // completion. The normal control-transfer continuation path already
  // waits between later PRE-qualified stages; cover this initial launch as
  // well. Keep it one-shot per pipe so interrupt endpoints are unaffected
  // after their first request is armed.
  if (pipe->ls_first_launch_pending) {
    pipe->ls_first_launch_pending = false;
    _buffer_wait_next_preamble_frame(pipe);
#ifdef USB_DWC_TT_DEBUG
    ESP_EARLY_LOGE(HCD_DWC_TAG, "LS first PRE launch frame=%" PRIu32 " HCFG=0x%08" PRIx32,
                   usb_dwc_ll_hfnum_get_frame_num(pipe->port->hal->dev), pipe->port->hal->dev->hcfg_reg.val);
#endif
  }
  ...
  switch (pipe->ep_char.type) {
    ...
    case USB_DWC_XFER_TYPE_INTR: {
      start_idx = 0;
      desc_list_len = (buffer_to_exec->flags.intr.zero_len_packet) ? buffer_to_exec->flags.intr.num_qtds + 1
                                                                   : buffer_to_exec->flags.intr.num_qtds;
      break;
    }
    default: {
      start_idx = 0;
  ...
  if (pipe->tt_enabled) {
    pipe->tt_split_phase = 1;
    _tt_set_ssplit(pipe);
    pipe->chan_obj->regs->hcchar_reg.chdis = 0;
    _tt_schedule_next_frame(pipe);
  }
  if (pipe->tt_enabled) {
    _tt_chan_activate(pipe, buffer_to_exec->xfer_desc_list, desc_list_len, start_idx);
  } else {
...
static void _buffer_exec_cont(pipe_t *pipe) {
  ...
  if (buffer_inflight->flags.ctrl.cur_stg == 0) {  // Just finished control stage
    ...
    next_dir_is_in =
        !buffer_inflight->flags.ctrl.data_stg_in;  // Status stage is always the opposite direction of data stage
    next_pid = 1;                                  // Status stage always has a PID of DATA1
  ...
  if (pipe->tt_enabled) {
    _tt_chan_activate(pipe, buffer_inflight->xfer_desc_list, XFER_LIST_LEN_CTRL, buffer_inflight->flags.ctrl.cur_stg);
  } else {
    usb_dwc_hal_chan_activate(pipe->chan_obj, buffer_inflight->xfer_desc_list, XFER_LIST_LEN_CTRL,
                              buffer_inflight->flags.ctrl.cur_stg);
  }
}
...
static inline void _buffer_parse_intr(dma_buffer_block_t *buffer, bool is_in, int mps) {
  ...
  if (is_in) {
    if (intr_stop_idx > 0) {  // This is an early stop (short packet)
      ...
      usb_dwc_hal_xfer_desc_parse(buffer->xfer_desc_list, buffer->flags.intr.num_qtds - 1, &last_packet_rem_len,
                                  &last_packet_desc_status);
      assert(last_packet_desc_status == USB_DWC_HAL_XFER_DESC_STS_SUCCESS);
      // All packets except last MUST be MPS. So just deduct the remaining length of the last packet to get actual
      // number of bytes
...
static inline void _buffer_parse_error(dma_buffer_block_t *buffer) {
  ...
  switch (buffer->status_flags.pipe_event) {
    case HCD_PIPE_EVENT_NONE:
      transfer->status =
          (buffer->status_flags.was_canceled) ? USB_TRANSFER_STATUS_CANCELED : USB_TRANSFER_STATUS_NO_DEVICE;
      break;
...
esp_err_t hcd_urb_enqueue(hcd_pipe_handle_t pipe_hdl, urb_t *urb) {
  ...
  // In case the pipe's interval is too long and there are too many ISOC packets, they might not fit into the transfer
  // descriptor list
  HCD_CHECK(!((pipe->ep_char.type == USB_DWC_XFER_TYPE_ISOCHRONOUS) &&
              (urb->transfer.num_isoc_packets * pipe->ep_char.periodic.interval > XFER_LIST_LEN_ISOC)),
...
urb_t *hcd_urb_dequeue(hcd_pipe_handle_t pipe_hdl) {
  ...
  if (pipe->num_urb_done > 0) {
    ...
    assert(urb->hcd_ptr == (void *) pipe &&
           urb->hcd_var == URB_HCD_STATE_DONE);  // The URB's reserved field should have been set to this pipe
    urb->hcd_ptr = NULL;
    urb->hcd_var = URB_HCD_STATE_IDLE;
    if (pipe->cs_flags.has_urb && pipe->num_urb_pending == 0 && pipe->num_urb_done == 0 &&
