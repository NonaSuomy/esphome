...
#define ENUM_SHORT_DESC_REQ_LEN \
  8  // Number of bytes to request when getting a short descriptor (just enough to get bMaxPacketSize0 or wTotalLength)
 ...
 *
 * Entry:
 * - These stages MUST BE LISTED IN THE ORDER OF THEIR EXECUTION as the enumeration will simply increment the current
 * stage
 ...
 * - The short descriptor stages are used to fetch the start particular descriptors that don't have a fixed length in
 * order to determine the full descriptors length
 * - Any state of Get String Descriptor could be STALLed by the device. In that case we just don't fetch them and treat
 * enumeration as successful
 */
typedef enum {
  ...
  ENUM_STAGE_SECOND_RESET, /**< Reset the device again (Workaround for old USB devices that get confused by the previous
                              short dev desc request). */
  ...
  ENUM_STAGE_CHECK_FULL_DEV_DESC, /**< Check the full dev desc, fill it into the device object in USBH. Save the string
                                     descriptor indexes*/
  ENUM_STAGE_SELECT_CONFIG, /**< Select configuration: select default ENUM_DEFAULT_CONFIGURATION_VALUE value or use
                               callback if ENABLE_ENUM_FILTER_CALLBACK enabled */
...
typedef struct {
  ...
  uint8_t
      bMaxPacketSize0; /**< Max packet size of the device's EP0. Read from bMaxPacketSize0 field of device descriptor */
  uint16_t wTotalLength; /**< Total length of device's configuration descriptor. Read from wTotalLength field of config
                            descriptor */
...
static uint8_t get_next_free_dev_addr(void) {
  ...
  while (1) {
    ...
    // We have a device with the same address on a bus, close device and request new addr, there should be no error with
    // closing
 ...
 * SetConfiguration(): bConfigurationValue field used as a parameter to the SetConfiguration() request. (refer to
 * section 9.6.3 Configuration)
 *
 * @return uint8_t
 */
 ...
 * This will call the enumeration filter callback (if enabled) and set the bConfigurationValue for the upcoming
...
static esp_err_t select_active_configuration(void) {
  ...
  // But some devices STALLed get configuration descriptor with bConfigurationValue = 1, even they have one
  // configuration with bValue = 1.
  ...
  if (!enum_proceed) {
    ESP_LOGW(ENUM_TAG, "[%d:%d] Abort request of enumeration process (%#x:%#x)",
             p_enum_driver->single_thread.parent_dev_addr, p_enum_driver->single_thread.parent_port_num,
...
static void control_request_general(enum_stage_t stage) {
  ...
  switch (stage) {
    ...
    case ENUM_STAGE_SET_ADDR: {
      ...
      USB_SETUP_PACKET_INIT_SET_ADDR((usb_setup_packet_t *) transfer->data_buffer,
                                     p_enum_driver->single_thread.dev_params.new_dev_addr);
    ...
    case ENUM_STAGE_GET_SHORT_CONFIG_DESC: {
      ...
      USB_SETUP_PACKET_INIT_GET_CONFIG_DESC((usb_setup_packet_t *) transfer->data_buffer, desc_index,
                                            ENUM_SHORT_DESC_REQ_LEN);
      transfer->num_bytes = sizeof(usb_setup_packet_t) + usb_round_up_to_mps(ENUM_SHORT_DESC_REQ_LEN, ctrl_ep_mps);
      // IN data stage should return exactly ENUM_SHORT_DESC_REQ_LEN bytes
      p_enum_driver->single_thread.expect_num_bytes = sizeof(usb_setup_packet_t) + ENUM_SHORT_DESC_REQ_LEN;
      break;
...
static void control_request_string(enum_stage_t stage) {
  ...
  switch (stage) {
    ...
    case ENUM_STAGE_GET_SHORT_SER_STR_DESC: {
      ...
      USB_SETUP_PACKET_INIT_GET_STR_DESC((usb_setup_packet_t *) transfer->data_buffer, index, langid,
...
static esp_err_t control_request(enum_stage_t stage) {
  ...
  if (ret != ESP_OK) {
    ESP_LOGE(ENUM_TAG, "[%d:%d] Control transfer submit error (%#x), stage '%s'",
             p_enum_driver->single_thread.parent_dev_addr, p_enum_driver->single_thread.parent_port_num, ret,
...
static esp_err_t control_response_handling(enum_stage_t stage) {
  ...
  if (ctrl_xfer->status != USB_TRANSFER_STATUS_COMPLETED) {
    if (ctrl_xfer->status == USB_TRANSFER_STATUS_STALL && stage >= ENUM_STAGE_CHECK_SHORT_LANGID_TABLE &&
        stage <= ENUM_STAGE_CHECK_FULL_SER_STR_DESC) {
      // String Descriptor request could be STALLed, if the device doesn't have them
    } else {
      ESP_LOGE(ENUM_TAG, "Bad transfer status %d: %s", ctrl_xfer->status, enum_stage_strings[stage]);
    }
    return ret;
  }
 ...
 *
 * When stage doesn't have a control transfer callback which request processing, request process should be triggered
 * immediately
 *
...
static bool set_next_stage(bool last_stage_pass) {
  ...
  while (1) {
    ...
    if (last_stage_pass) {
      ...
      ESP_LOGD(ENUM_TAG, "[%d:%d] %s OK", p_enum_driver->single_thread.parent_dev_addr,
               p_enum_driver->single_thread.parent_port_num, enum_stage_strings[last_stage]);
      // Get next stage
      if (last_stage == ENUM_STAGE_COMPLETE || last_stage == ENUM_STAGE_CANCEL) {
      ...
      assert(last_stage != ENUM_STAGE_SET_ADDR_RECOVERY && last_stage != ENUM_STAGE_SELECT_CONFIG &&
             last_stage != ENUM_STAGE_SECOND_RESET && last_stage != ENUM_STAGE_SECOND_RESET_COMPLETE &&
             last_stage != ENUM_STAGE_COMPLETE && last_stage != ENUM_STAGE_CANCEL);
      ...
      switch (last_stage) {
        ...
        default:
          ...
          ESP_LOGE(ENUM_TAG, "[%d:%d] %s FAILED", p_enum_driver->single_thread.parent_dev_addr,
                   p_enum_driver->single_thread.parent_port_num, enum_stage_strings[last_stage]);
...
esp_err_t enum_start(unsigned int uid) {
  ...
  ESP_LOGD(ENUM_TAG, "[%d:%d] Start processing, device address %d", parent_dev_addr, dev_info.parent.port_num, 0);
  ...
  // Device params
  memset(&p_enum_driver->single_thread.dev_params, 0, sizeof(enum_device_params_t));
  p_enum_driver->single_thread.dev_params.bMaxPacketSize0 =
      (dev_info.speed == USB_SPEED_LOW) ? ENUM_WORST_CASE_MPS_LS : ENUM_WORST_CASE_MPS_FS_HS;
...
esp_err_t enum_cancel(unsigned int uid) {
  ...
  if (old_stage == ENUM_STAGE_IDLE || old_stage == ENUM_STAGE_CANCEL) {
    // Nothing to cancel
    return ESP_OK;
  }
  ...
  ESP_LOGV(ENUM_TAG, "[%d:%d] Cancel at %s", p_enum_driver->single_thread.parent_dev_addr,
           p_enum_driver->single_thread.parent_port_num, enum_stage_strings[old_stage]);
  ...
  // SECOND_RESET_COMPLETE is the exceptional stage, as it awaits the notification after port reset completion via the
  // enum_proceed() call. Meanwhile, the device could be detached during the reset, thus the device disconnect comes
  // instead of reset completion.
