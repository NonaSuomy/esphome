...
const usb_standard_desc_t *usb_parse_next_descriptor(const usb_standard_desc_t *cur_desc, uint16_t wTotalLength,
                                                     int *offset) {
...
const usb_standard_desc_t *usb_parse_next_descriptor_of_type(const usb_standard_desc_t *cur_desc, uint16_t wTotalLength,
                                                             uint8_t bDescriptorType, int *offset) {
  assert(cur_desc != NULL && offset != NULL);
...
int usb_parse_interface_number_of_alternate(const usb_config_desc_t *config_desc, uint8_t bInterfaceNumber) {
  ...
  const usb_intf_desc_t *next_intf_desc = (const usb_intf_desc_t *) usb_parse_next_descriptor_of_type(
      (const usb_standard_desc_t *) first_intf_desc, config_desc->wTotalLength, USB_B_DESCRIPTOR_TYPE_INTERFACE,
      &offset);
...
const usb_intf_desc_t *usb_parse_interface_descriptor(const usb_config_desc_t *config_desc, uint8_t bInterfaceNumber,
                                                      uint8_t bAlternateSetting, int *offset) {
  assert(config_desc != NULL);

  // Walk to first interface descriptor of bInterfaceNumber
  int offset_temp = 0;
  const usb_intf_desc_t *next_intf_desc = (const usb_intf_desc_t *) usb_parse_next_descriptor_of_type(
      (const usb_standard_desc_t *) config_desc, config_desc->wTotalLength, USB_B_DESCRIPTOR_TYPE_INTERFACE,
      &offset_temp);
  while (next_intf_desc != NULL) {
    if (next_intf_desc->bInterfaceNumber == bInterfaceNumber) {
...
const usb_ep_desc_t *usb_parse_endpoint_descriptor_by_index(const usb_intf_desc_t *intf_desc, int index,
  ...
  for (int i = 0; i <= index; i++) {
    next_desc = usb_parse_next_descriptor_of_type((const usb_standard_desc_t *) next_desc, wTotalLength,
                                                  USB_B_DESCRIPTOR_TYPE_ENDPOINT, &offset_temp);
    if (next_desc == NULL) {
      ep_found = false;
      break;
    }
...
const usb_ep_desc_t *usb_parse_endpoint_descriptor_by_address(const usb_config_desc_t *config_desc,
                                                              ...
                                                              uint8_t bEndpointAddress, int *offset) {
  assert(config_desc != NULL);
...
static void print_ep_desc(const usb_ep_desc_t *ep_desc) {
  ...
  printf("\t\tbEndpointAddress 0x%x\tEP %d %s\n", ep_desc->bEndpointAddress, USB_EP_DESC_GET_EP_NUM(ep_desc),
