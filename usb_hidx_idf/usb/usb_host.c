...
typedef struct {
  ...
  struct {
    ...
    void *hub_client;   // Pointer to External Hub driver (acting as a client). Used to reroute completed USBH control
                        // transfers. NULL, when External Hub Driver not available.
  } constant;
} host_lib_t;
...
static void send_event_msg_to_clients(const usb_host_client_event_msg_t *event_msg, bool send_to_all,
                                      uint8_t opened_dev_addr) {
  // Lock client list
...
static void usbh_event_callback(usbh_event_data_t *event_data, void *arg) {
  switch (event_data->event) {
    ...
    case USBH_EVENT_DEV_FREE: {
      ...
      hub_port_recycle(event_data->dev_free_data.parent_dev_hdl, event_data->dev_free_data.port_num,
...
static bool endpoint_callback(usbh_ep_handle_t ep_hdl, usbh_ep_event_t ep_event, void *user_arg, bool in_isr) {
  ...
  HOST_ENTER_CRITICAL_SAFE();
  // Store the event to be handled later. Note that we allow overwriting of events because more severe will halt the
  // pipe prevent any further events.
...
esp_err_t usb_host_install(const usb_host_config_t *config) {
  ...
  */
  ...
  if (config->fifo_settings_custom.rx_fifo_lines != 0 || config->fifo_settings_custom.nptx_fifo_lines != 0 ||
...
esp_err_t usb_host_uninstall(void) {
  ...
  HOST_CHECK_FROM_CRIT(p_host_lib_obj->dynamic.process_pending_flags == 0 &&
                           p_host_lib_obj->dynamic.lib_event_flags == 0 && p_host_lib_obj->dynamic.flags.val == 0,
...
esp_err_t usb_host_lib_handle_events(TickType_t timeout_ticks, uint32_t *event_flags_ret) {
  ...
  esp_err_t ret =
      (timeout_ticks == 0) ? ESP_OK : ESP_ERR_TIMEOUT;  // We don't want to return ESP_ERR_TIMEOUT if we aren't blocking
...
esp_err_t usb_host_client_deregister(usb_host_client_handle_t client_hdl) {
  ...
  bool can_deregister;
  if (!TAILQ_EMPTY(&client_obj->dynamic.pending_ep_tailq) || !TAILQ_EMPTY(&client_obj->dynamic.idle_ep_tailq) ||
      !TAILQ_EMPTY(&client_obj->dynamic.done_ctrl_xfer_tailq) || client_obj->dynamic.flags.handling_events ||
      client_obj->dynamic.flags.taking_mux || client_obj->dynamic.flags.num_intf_claimed != 0 ||
      client_obj->dynamic.num_done_ctrl_xfer != 0 || client_obj->dynamic.opened_dev_addr_map[0] != 0 ||
      client_obj->dynamic.opened_dev_addr_map[1] != 0 || client_obj->dynamic.opened_dev_addr_map[2] != 0 ||
...
esp_err_t usb_host_device_free_all(void) {
  HOST_ENTER_CRITICAL();
  HOST_CHECK_FROM_CRIT(p_host_lib_obj->dynamic.flags.num_clients == 0,
                       ESP_ERR_INVALID_STATE);  // All clients must have been deregistered
  HOST_EXIT_CRITICAL();
  esp_err_t ret;
#if ENABLE_USB_HUBS
  hub_notify_all_free();
  ...
  // If ESP_ERR_NOT_FINISHED is returned, caller must wait for USB_HOST_LIB_EVENT_FLAGS_ALL_FREE to confirm all devices
  // are free
  return ret;
}
...
static esp_err_t get_config_desc_transfer(usb_host_client_handle_t client_hdl, usb_transfer_t *ctrl_transfer,
                                          const int bConfigurationValue, const int num_bytes) {
...
esp_err_t usb_host_get_config_desc(usb_host_client_handle_t client_hdl, usb_device_handle_t dev_hdl,
  ...
  const usb_config_desc_t *config_desc_short =
      (usb_config_desc_t *) (ctrl_transfer->data_buffer + sizeof(usb_setup_packet_t));
  ...
  const usb_config_desc_t *config_desc_full =
      (usb_config_desc_t *) (ctrl_transfer->data_buffer + sizeof(usb_setup_packet_t));
...
static esp_err_t ep_wrapper_alloc(usb_device_handle_t dev_hdl, const usb_ep_desc_t *ep_desc, interface_t *intf_obj,
                                  ep_wrapper_t **ep_wrap_ret) {
...
static interface_t *interface_alloc(client_t *client_obj, usb_device_handle_t dev_hdl,
                                    const usb_intf_desc_t *intf_desc) {
  interface_t *intf_obj = heap_caps_calloc(1, sizeof(interface_t) + (sizeof(ep_wrapper_t *) * intf_desc->bNumEndpoints),
                                           MALLOC_CAP_DEFAULT);
  if (intf_obj == NULL) {
    return NULL;
  }
...
static esp_err_t interface_claim(client_t *client_obj, usb_device_handle_t dev_hdl,
                                 const usb_config_desc_t *config_desc, uint8_t bInterfaceNumber,
                                 uint8_t bAlternateSetting, interface_t **intf_obj_ret) {
  esp_err_t ret;
  // We need to walk to configuration descriptor to find the correct interface descriptor, and each of its constituent
  // endpoint descriptors Find the interface descriptor and allocate the interface object
  int offset_intf;
  const usb_intf_desc_t *intf_desc =
      usb_parse_interface_descriptor(config_desc, bInterfaceNumber, bAlternateSetting, &offset_intf);
  if (intf_desc == NULL) {
  ...
  for (int i = 0; i < intf_desc->bNumEndpoints; i++) {
    ...
    const usb_ep_desc_t *ep_desc =
        usb_parse_endpoint_descriptor_by_index(intf_desc, i, config_desc->wTotalLength, &offset_ep);
    if (ep_desc == NULL) {
...
static esp_err_t interface_release(client_t *client_obj, usb_device_handle_t dev_hdl, uint8_t bInterfaceNumber) {
  ...
  TAILQ_FOREACH(intf_obj_iter, &client_obj->mux_protected.interface_tailq, mux_protected.tailq_entry) {
    if (intf_obj_iter->constant.dev_hdl == dev_hdl &&
        intf_obj_iter->constant.intf_desc->bInterfaceNumber == bInterfaceNumber) {
