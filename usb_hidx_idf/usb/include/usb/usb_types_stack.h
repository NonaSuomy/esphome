...
struct usb_transfer_s {
  ...
  const int num_isoc_packets; /**< Only relevant to Isochronous. Number of service periods (i.e., intervals) to transfer
                                 data buffer over. */
 ...
 * - For MSC Bulk Only Transport class, the Host MUST NEVER send a ZLP. Bulk transfer boundaries are determined by the
 * CBW and CSW instead
 * - For CDC Ethernet, the Host MUST ALWAYS send a ZLP if a segment (i.e., a transfer) is a multiple of MPS (See 3.3.1
 * Segment Delineation)
 *
 ...
 * MPS) or the endpoint sends a short packet to the Host
 */
#define USB_TRANSFER_FLAG_ZERO_PACK \
  0x01 /**< (For bulk OUT only). Indicates that a bulk OUT transfers should always terminate with a short packet, even \
          if it means adding an extra zero length packet */
