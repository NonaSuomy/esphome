...
typedef struct {
  ...
  usb_host_enum_filter_cb_t
      enum_filter_cb; /**< Enumeration filter callback. Enable CONFIG_USB_HOST_ENABLE_ENUM_FILTER_CALLBACK
  ...
  unsigned
      peripheral_map; /**< Selects the USB peripheral(s) to use.
                           - On targets with multiple USB peripherals, this field can be used to specify which ones to
                         enable.
 ...
 * - The resulting data_buffer_size can be bigger that the requested size. This is to ensure that the data buffer is
 * cache aligned
