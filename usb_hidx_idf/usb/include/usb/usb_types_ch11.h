...
typedef enum {
  ...
  USB_B_REQUEST_HUB_CLEAR_FEATURE =
      0x01, /**< Clearing a feature disables that feature or starts a process associated with the feature. */
  ...
  USB_B_REQUEST_HUB_CLEAR_TT_BUFFER =
      0x08, /**< This request clears the state of a Transaction Translator(TT) bulk/control buffer after it has been
               left in a busy state due to high-speed errors. */
 ...
 */
typedef enum {
  USB_PORT_STATE_NOT_CONFIGURED = 0x00, /**< The hub is not configured. A port transitions to and remains in this state
                                           whenever the value of the hub configuration is zero. */
  USB_PORT_STATE_POWERED_OFF,           /**< Powered_off: Port requires explicit request to transition. */
  USB_PORT_STATE_DISCONNECTED, /**< In the Disconnected state, the port’s differential transmitter and receiver are
                                  disabled and only connection detection is possible.*/
  USB_PORT_STATE_DISABLED,  /**< A port in the Disabled state will not propagate signaling in either the upstream or the
                               downstream direction */
  USB_PORT_STATE_RESETTING, /**< The duration of the Resetting state is nominally 10 ms to 20 ms (10 ms is preferred).
                             */
  USB_PORT_STATE_ENABLED, /**< While in this state, the output of the port’s differential receiver is available to the
                             Hub Repeater so that appropriate signaling transitions can establish upstream
                             connectivity*/
  USB_PORT_STATE_TRANSMIT, /**< This state is entered from the Enabled state on the transition of the Hub Repeater to
                              the WFEOPFU state */
  USB_PORT_STATE_TRANSMIT_R, /**< When in this state, the port repeats the resume ‘K’ at the upstream facing port to the
                                downstream facing port. */
  USB_PORT_STATE_SUSPENDED, /**< While a port is in the Suspended state, the port's differential transmitter is
                               disabled. */
  ...
  USB_PORT_STATE_RESTART_S, /**< A port enters the Restart_S state from the Suspended state when an SE0 or ‘K’ is seen
                               at the port and the Receiver is in the Suspended state */
  USB_PORT_STATE_RESTART_E, /**< A port enters the Restart_E state from the Enabled state when an ‘SE0’ or ‘K’ is seen
                               at the port and the Receiver is in the Suspended state. */
...
#define USB_W_HUB_CHARS_TTTT_8_BITS \
...
typedef struct {
  union {
    struct {
      ...
      uint8_t PORT_OVER_CURRENT : 1; /**< 0 = All no over-current condition exists on this port. 1 = An over-current
      ...
      uint8_t PORT_POWER : 1; /**< 0 = This port is in the Powered-off state. 1 = This port is not in the Powered-off
                                 state. */
      uint8_t PORT_LOW_SPEED : 1; /**< 0 = Full-speed or High-speed device attached to this port (determined by bit 10).
                                     1 = Low-speed device attached to this port.*/
      uint8_t PORT_HIGH_SPEED : 1; /**< 0 = Full-speed device attached to this port. 1 = High-speed device attached to
                                      this port. */
      uint8_t PORT_TEST : 1; /**< 0 = This port is not in the Port Test Mode. 1 = This port is in Port Test Mode. */
      uint8_t PORT_INDICATOR : 1; /**< 0 = Port indicator displays default colors. 1 = Port indicator displays software
                                     controlled color. */
  ...
  union {
    struct {
      uint8_t C_PORT_CONNECTION : 1; /**< 0 = No change has occurred to Current Connect status. 1 = Current Connect
                                        status has changed. */
      uint8_t
          C_PORT_ENABLE : 1; /**< This field is set to one when a port is disabled because of a Port_Error condition */
      uint8_t C_PORT_SUSPEND : 1;      /**< 0 = No change. 1 = Resume complete. */
      uint8_t C_PORT_OVER_CURRENT : 1; /**< 0 = No change has occurred to Over-Current Indicator. 1 = Over-Current
                                          Indicator has changed. */
      uint8_t C_PORT_RESET : 1; /**< This field is set when reset processing on this port is complete. 0 = No change. 1
                                   = Reset complete.*/
...
typedef struct {
  union {
    struct {
      ...
      uint8_t HUB_OVER_CURRENT : 1; /**< 0 = No over-current condition currently exists. 1 = A hub over-current
                                       condition exists.*/
  ...
  union {
    struct {
      uint8_t C_HUB_LOCAL_POWER : 1;  /**< 0 = No change has occurred to Local Power Status. 1 = Local Power Status has
                                         changed.*/
      uint8_t C_HUB_OVER_CURRENT : 1; /**< 0 = No change has occurred to the Over-Current Status. 1 = Over-Current
                                         Status has changed.*/
      uint16_t RESERVED : 14;         /**< Reserved fields */
    };
...
typedef struct {
  ...
  uint8_t bPwrOn2PwrGood; /**< Time (in 2 ms intervals) from the time the power-on sequence begins on a port until power
                             is good on that port */
 ...
 */
#define USB_SETUP_PACKET_GET_PORT(setup_pkt_ptr) ({ (setup_pkt_ptr)->wIndex; })
