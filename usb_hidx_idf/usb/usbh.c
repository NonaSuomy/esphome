...
// Device action flags. LISTED IN THE ORDER THEY SHOULD BE HANDLED IN within usbh_process(). Some actions are mutually
// exclusive
typedef enum {
  DEV_ACTION_EPn_HALT_FLUSH =
      (1 << 0),  // Halt all non-default endpoints then flush them (called after a device gone is gone)
...
typedef struct {
  struct {
    ...
    const usb_ep_desc_t
        *ep_desc; /**< This just stores a pointer endpoint descriptor inside the device's "config_desc" */
...
struct device_s {
  struct {
    ...
    union {
      struct {
        ...
        uint32_t enum_lock : 1;   /**< Device is locked for enumeration. Enum information (e.g., address, device/config
                                     desc etc) may change */
  ...
  struct {
    ...
    usb_device_handle_t
        parent_dev_hdl;      /**< Device's parent device handle. NULL if device is connected to the root port */
    ...
    uint8_t tt_hub_addr;     /**< Nearest high-speed TT hub address */
    uint8_t tt_port_num;     /**< Downstream port on the TT hub */
    bool tt_enabled;         /**< Device is routed through a high-speed TT */
...
typedef struct {
  struct {
    ...
    TAILQ_HEAD(tailhead_devs_cb, device_s)
    devs_pending_tailq; /**< Tailq of devices that need to have their cb called */
  } dynamic;            /**< Dynamic members. Require a critical section */
...
static bool transfer_check_usb_compliance(usb_transfer_t *transfer, usb_transfer_type_t type, unsigned int mps,
                                          bool is_in) {
  if (type == USB_TRANSFER_TYPE_CTRL) {
    ...
    if (mismatch) {
      ESP_LOGE(USBH_TAG, "usb_transfer_t num_bytes %d and usb_setup_packet_t wLength %d mismatch", transfer->num_bytes,
               setup_pkt->wLength);
      return false;
    }
...
static esp_err_t endpoint_alloc(device_t *dev_obj, const usb_ep_desc_t *ep_desc, usbh_ep_config_t *ep_config,
                                endpoint_t **ep_obj_ret) {
  esp_err_t ret;
  endpoint_t *ep_obj;
  ...
  hcd_pipe_config_t pipe_config = {
      ...
      .tt_hub_addr = dev_obj->constant.tt_hub_addr,
      .tt_port_num = dev_obj->constant.tt_port_num,
      .tt_enabled = dev_obj->constant.tt_enabled,
  };
...
static esp_err_t device_alloc(usbh_dev_params_t *params, device_t **dev_obj_ret) {
  ...
  usb_device_info_t parent_info = {0};
  esp_err_t parent_info_ret = ESP_ERR_NOT_FOUND;
  uint8_t parent_addr = 0;
  if (params->speed != USB_SPEED_HIGH && params->parent_dev_hdl != NULL &&
      (parent_info_ret = usbh_dev_get_info(params->parent_dev_hdl, &parent_info)) == ESP_OK &&
      parent_info.speed == USB_SPEED_HIGH) {
    // Split transactions are only valid when the parent hub itself is
    // high speed.  The P4 HS root stays at high speed and routes each
    // full-/low-speed child through the hub's transaction translator.
    if (usbh_dev_get_addr(params->parent_dev_hdl, &parent_addr) == ESP_OK) {
      pipe_config.tt_hub_addr = parent_addr;
      pipe_config.tt_port_num = params->parent_port_num;
      pipe_config.tt_enabled = true;
      ESP_LOGD(USBH_TAG, "FS device via TT hub addr=%u port=%u", pipe_config.tt_hub_addr, pipe_config.tt_port_num);
    }
  }
  if (params->parent_dev_hdl != NULL) {
    (void) usbh_dev_get_addr(params->parent_dev_hdl, &parent_addr);
    ESP_LOGE(USBH_TAG,
             "Device alloc uid=%u child_speed=%d parent_speed=%d parent_addr=%u"
             " parent_info=%s tt=%s tt_port=%u",
             params->uid, params->speed, parent_info.speed, parent_addr, esp_err_to_name(parent_info_ret),
             pipe_config.tt_enabled ? "yes" : "no", pipe_config.tt_port_num);
  }
  ...
  dev_obj->constant.tt_hub_addr = pipe_config.tt_hub_addr;
  dev_obj->constant.tt_port_num = pipe_config.tt_port_num;
  dev_obj->constant.tt_enabled = pipe_config.tt_enabled;
...
static bool epN_pipe_callback(hcd_pipe_handle_t pipe_hdl, hcd_pipe_event_t pipe_event, void *user_arg, bool in_isr) {
  ...
  return ep_obj->constant.ep_cb((usbh_ep_handle_t) ep_obj, (usbh_ep_event_t) pipe_event, ep_obj->constant.ep_cb_arg,
                                in_isr);
}
...
static inline void handle_free(device_t *dev_obj) {
  ...
  usbh_event_data_t event_data = {.event = USBH_EVENT_DEV_FREE,
                                  .dev_free_data = {
                                      .dev_uid = dev_uid,
  ...
  */
  TAILQ_FOREACH(dev_obj, &p_usbh_obj->dynamic.devs_idle_tailq, dynamic.tailq_entry) {
    if (num_filled < list_len) {
      if (!dev_obj->dynamic.flags.enum_lock && dev_obj->dynamic.state == USB_DEVICE_STATE_CONFIGURED &&
...
esp_err_t usbh_devs_remove(unsigned int uid) {
  ...
  if (dev_obj->dynamic.open_count == 0) {
    ...
    call_proc_req_cb = _dev_set_actions(
        dev_obj, DEV_ACTION_EPn_HALT_FLUSH | DEV_ACTION_EP0_FLUSH | DEV_ACTION_EP0_DEQUEUE | DEV_ACTION_PROP_GONE_EVT);
  }
  ret = ESP_OK;
exit:
  USBH_EXIT_CRITICAL();
...
esp_err_t usbh_ep_alloc(usb_device_handle_t dev_hdl, usbh_ep_config_t *ep_config, usbh_ep_handle_t *ep_hdl_ret) {
  ...
  const usb_ep_desc_t *ep_desc =
      usb_parse_endpoint_descriptor_by_address(dev_obj->constant.config_desc, ep_config->bInterfaceNumber,
                                               ep_config->bAlternateSetting, ep_config->bEndpointAddress, NULL);
  if (ep_desc == NULL) {
...
esp_err_t usbh_dev_submit_ctrl_urb(usb_device_handle_t dev_hdl, urb_t *urb) {
  ...
  USBH_CHECK(transfer_check_usb_compliance(&(urb->transfer), USB_TRANSFER_TYPE_CTRL, mps, xfer_is_in),
             ESP_ERR_INVALID_ARG);
...
esp_err_t usbh_ep_enqueue_urb(usbh_ep_handle_t ep_hdl, urb_t *urb) {
  ...
  USBH_CHECK(transfer_check_usb_compliance(&(urb->transfer), USB_EP_DESC_GET_XFERTYPE(ep_obj->constant.ep_desc),
