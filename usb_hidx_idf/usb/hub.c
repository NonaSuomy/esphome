...
static esp_err_t dev_tree_node_reset_completed(usb_device_handle_t parent_dev_hdl, uint8_t parent_port_num) {
  ...
  TAILQ_FOREACH(dev_tree_iter, &p_hub_driver_obj->single_thread.dev_nodes_tailq, tailq_entry) {
    if (dev_tree_iter->parent_dev_hdl == parent_dev_hdl && dev_tree_iter->parent_port_num == parent_port_num) {
...
static bool root_port_callback(hcd_port_handle_t port_hdl, hcd_port_event_t port_event, void *user_arg, bool in_isr) {
  ...
  return p_hub_driver_obj->constant.proc_req_cb(USB_PROC_REQ_SOURCE_HUB, in_isr,
                                                p_hub_driver_obj->constant.proc_req_cb_arg);
}
...
static void ext_port_event_callback(ext_port_hdl_t port_hdl, ext_port_event_data_t *event_data, void *arg) {
  ...
  switch (event_data->event) {
    case EXT_PORT_CONNECTED:
      ...
      if (ext_hub_port_get_speed(ext_hub_hdl, event_data->connected.parent_port_num, &port_speed) != ESP_OK) {
        goto new_ds_dev_err;
      }
      ...
      // Full-/low-speed devices behind a high-speed hub are routed through
      // the hub's transaction translator. The HCD carries the parent hub
      // address and port into DWC split transactions during pipe creation.

      if (dev_tree_node_new(event_data->connected.parent_dev_hdl, event_data->connected.parent_port_num, port_speed) !=
          ESP_OK) {
    ...
    case EXT_PORT_RESET_COMPLETED:
      ESP_ERROR_CHECK(dev_tree_node_reset_completed(event_data->reset_completed.parent_dev_hdl,
                                                    event_data->reset_completed.parent_port_num));
      break;
...
static void root_port_handle_events(hcd_port_handle_t root_port_hdl) {
  ...
  ESP_LOGI(HUB_DRIVER_TAG, "Root port HCD event=%d state=%d", port_event, hcd_port_get_state(root_port_hdl));
  switch (port_event) {
    case HCD_PORT_EVENT_NONE:
      // Nothing to do
      break;
    case HCD_PORT_EVENT_CONNECTION: {
      esp_err_t reset_ret = hcd_port_command(root_port_hdl, HCD_PORT_CMD_RESET);
      ESP_LOGI(HUB_DRIVER_TAG, "Root port reset result=%s state=%d", esp_err_to_name(reset_ret),
               hcd_port_get_state(root_port_hdl));
      if (reset_ret != ESP_OK) {
      ...
      usb_speed_t speed;
      esp_err_t speed_ret = hcd_port_get_speed(p_hub_driver_obj->constant.root_port_hdl, &speed);
      ESP_LOGI(HUB_DRIVER_TAG, "Root port speed result=%s speed=%d", esp_err_to_name(speed_ret), speed);
      if (speed_ret != ESP_OK) {
        goto new_dev_err;
      }

      esp_err_t dev_ret = dev_tree_node_new(NULL, 0, speed);
      ESP_LOGI(HUB_DRIVER_TAG, "Root device create result=%s", esp_err_to_name(dev_ret));
      if (dev_ret != ESP_OK) {
    ...
    reset_err:
      // A root reset failure must be followed by HCD recovery. Without
      // this, a failed P4 reset leaves the HCD in RESETTING/DISABLED and
      // subsequent devices cannot generate a usable connection event.
      HUB_DRIVER_ENTER_CRITICAL();
      p_hub_driver_obj->dynamic.port_reqs |= PORT_REQ_RECOVER;
      p_hub_driver_obj->dynamic.flags.actions |= HUB_DRIVER_ACTION_ROOT_REQ;
      HUB_DRIVER_EXIT_CRITICAL();
      p_hub_driver_obj->constant.proc_req_cb(USB_PROC_REQ_SOURCE_HUB, false,
                                             p_hub_driver_obj->constant.proc_req_cb_arg);
      break;
    }
...
esp_err_t hub_uninstall(void) {
  HUB_DRIVER_ENTER_CRITICAL();
  HUB_DRIVER_CHECK_FROM_CRIT(p_hub_driver_obj != NULL, ESP_ERR_INVALID_STATE);
  HUB_DRIVER_CHECK_FROM_CRIT(p_hub_driver_obj->dynamic.root_port_state == ROOT_PORT_STATE_NOT_POWERED,
